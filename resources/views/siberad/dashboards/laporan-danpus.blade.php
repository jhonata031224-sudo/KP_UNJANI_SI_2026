...
<title>{{ $satuan->nama ?? 'Pimpinan' }} — {{ $pengaturan?->namaSistem() ?? 'SIBERAD' }}</title>
...
#kirimSuratModal .surat-tindakan-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:6px;border:1px solid transparent;border-radius:10px}
#kirimSuratModal .surat-tindakan-check .check-label{font-size:11px;line-height:1.3;text-transform:none;letter-spacing:0;font-weight:600;color:var(--text)}
@media(max-width:640px){#kirimSuratModal .surat-tindakan-grid{grid-template-columns:1fr}}
</style>
@php
  // PENTING: @json() Blade motong argumennya pakai explode(',') mentah
  // (buat pisahin opsi encoding/depth opsional) tanpa peduli nesting --
  // jadi array literal dgn banyak koma di dalam closure ->map(fn...) bikin
  // hasil PHP-nya rusak (ParseError). Makanya dirakit dulu di variabel
  // biasa, baru di-@json() sebagai satu variabel tunggal.
  $__suratTujuanOptions = $satuanSuratTujuanPilihan->map(fn($st) => ['id' => (string) $st->id, 'name' => $st->nama, 'kode' => $st->kode])->values();
  // Surat Keluar Danpus SELALU ke Wadan -- field Tujuan dihapus dari form,
  // ID satuan Wadan dikunci di sini biar gak bisa dikirim ke satuan lain.
  // Ini KHUSUS Danpus->Wadan, alur satuan lain (laporan-role/wadan) TETAP pakai combobox Tujuan biasa.
  $satuanTujuanTetapWadan = \App\Models\Satuan::where('kode', 'WADAN')->first();
@endphp
<div class="report-modal" id="kirimSuratModal"><div class="report-modal-card" style="width:min(860px,100%)"><div class="report-modal-head"><div style="display:flex;align-items:flex-start;gap:12px;min-width:0"><span class="surat-modal-icon"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8Z"></path><path d="M14 2v6h6"></path><path d="M12 12v6"></path><path d="M9 15h6"></path></svg></span><span style="min-width:0"><h3 style="margin:0 0 4px">Buat Surat Baru</h3><p style="margin:0;font-size:12px;color:var(--text-muted)">Dari {{ $satuan->nama }}</p></span></div><button type="button" class="report-modal-close" id="kirimSuratClose" aria-label="Tutup"><svg viewBox="0 0 24 24" stroke-linecap="round" stroke-linejoin="round"><path d="M6 6l12 12M18 6L6 18"></path></svg></button></div><form method="POST" action="{{ route('laporan-surat.store') }}" enctype="multipart/form-data" id="kirimSuratForm">@csrf<div class="surat-form-grid"><div class="surat-form-col"><div class="surat-section-label">Informasi Surat</div><input type="hidden" id="surat_tujuan" name="tujuan_satuan_id" value="{{ $satuanTujuanTetapWadan->id ?? '' }}"><div class="form-field full"><label for="surat_perihal">Perihal</label><input id="surat_perihal" name="perihal" maxlength="255" required autocomplete="off" placeholder="Judul singkat surat"></div><div class="form-field full"><label for="surat_kategori">Kategori</label><input id="surat_kategori" name="kategori" maxlength="255" required autocomplete="off" placeholder="Contoh: Undangan, Pemberitahuan, Koordinasi"></div><div class="form-field full"><label>Jenis Surat</label><div class="priority-toggle"><label class="priority-option prio-biasa"><input type="radio" name="prioritas" value="Biasa" required><span>Biasa</span></label><label class="priority-option prio-kilat"><input type="radio" name="prioritas" value="Kilat" required><span>Kilat</span></label><label class="priority-option prio-rahasia"><input type="radio" name="prioritas" value="Rahasia" required><span>Rahasia</span></label></div><p class="surat-prioritas-hint" style="margin:6px 0 0;font-size:11px;color:var(--text-muted)">Rahasia: ringkasan surat tidak akan ditampilkan ke penerima. Jenis surat hanya ditentukan sekali saat surat dibuat dan tidak bisa diubah setelah dikirim.</p></div><div class="form-field full" id="suratDeadlineField" style="display:none"><label>Deadline Surat Kilat</label><div class="danpus-deadline-split"><div class="danpus-deadline-part"><span class="danpus-deadline-sublabel">Tanggal</span><div class="danpus-picker" id="suratDeadlineDatePickerBox"><input type="text" id="suratDeadlineDateInput" class="danpus-picker-input" readonly autocomplete="off" placeholder="Pilih tanggal"><input type="text" id="suratDeadlineDateProxy" required style="position:absolute;opacity:0;width:1px;height:1px;pointer-events:none"><button type="button" class="danpus-picker-icon" aria-label="Pilih tanggal"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="4" width="18" height="18" rx="2"></rect><path d="M16 2v4"></path><path d="M8 2v4"></path><path d="M3 10h18"></path></svg></button></div></div><div class="danpus-deadline-part"><span class="danpus-deadline-sublabel">Jam</span><div class="danpus-picker" id="suratDeadlineTimePickerBox"><input type="text" id="suratDeadlineTimeInput" class="danpus-picker-input" readonly autocomplete="off" placeholder="Pilih jam"><input type="text" id="suratDeadlineTimeProxy" required style="position:absolute;opacity:0;width:1px;height:1px;pointer-events:none"><button type="button" class="danpus-picker-icon" aria-label="Pilih jam"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="9"></circle><path d="M12 7v5l3 3"></path></svg></button></div></div></div><input type="hidden" id="suratDeadlineHidden" name="deadline_at"><p style="margin:6px 0 0;font-size:11px;color:var(--text-muted)">Batas waktu surat ini harus ditindaklanjuti penerima. Wajib diisi khusus jenis surat Kilat.</p></div><div class="form-field full"><label for="surat_disposisi">Disposisi</label><select id="surat_disposisi" disabled style="cursor:not-allowed;opacity:.7"><option selected>WADAN</option></select><input type="hidden" name="disposisi" value="WADAN"></div><div class="form-field full"><label>Tindakan</label><div class="surat-tindakan-grid" id="suratTindakanGrid"><label class="danpus-request-check surat-tindakan-check"><input type="checkbox" name="tindakan[]" value="SESUAI JUK KOMANDAN"><span class="check-label">SESUAI JUK KOMANDAN</span><span class="check-mark"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><path d="M5 13l4 4L19 7"/></svg></span></label><label class="danpus-request-check surat-tindakan-check"><input type="checkbox" name="tindakan[]" value="ACC"><span class="check-label">ACC</span><span class="check-mark"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><path d="M5 13l4 4L19 7"/></svg></span></label><label class="danpus-request-check surat-tindakan-check"><input type="checkbox" name="tindakan[]" value="PEDOMAN"><span class="check-label">PEDOMAN</span><span class="check-mark"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><path d="M5 13l4 4L19 7"/></svg></span></label><label class="danpus-request-check surat-tindakan-check"><input type="checkbox" name="tindakan[]" value="SEBAGAI BAHAN"><span class="check-label">SEBAGAI BAHAN</span><span class="check-mark"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><path d="M5 13l4 4L19 7"/></svg></span></label><label class="danpus-request-check surat-tindakan-check"><input type="checkbox" name="tindakan[]" value="PELAJARI"><span class="check-label">PELAJARI</span><span class="check-mark"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><path d="M5 13l4 4L19 7"/></svg></span></label><label class="danpus-request-check surat-tindakan-check"><input type="checkbox" name="tindakan[]" value="KOORDINASIKAN"><span class="check-label">KOORDINASIKAN</span><span class="check-mark"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><path d="M5 13l4 4L19 7"/></svg></span></label><label class="danpus-request-check surat-tindakan-check"><input type="checkbox" name="tindakan[]" value="CATAT"><span class="check-label">CATAT</span><span class="check-mark"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><path d="M5 13l4 4L19 7"/></svg></span></label><label class="danpus-request-check surat-tindakan-check"><input type="checkbox" name="tindakan[]" value="HADIR"><span class="check-label">HADIR</span><span class="check-mark"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><path d="M5 13l4 4L19 7"/></svg></span></label><label class="danpus-request-check surat-tindakan-check"><input type="checkbox" name="tindakan[]" value="WAKILI"><span class="check-label">WAKILI</span><span class="check-mark"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><path d="M5 13l4 4L19 7"/></svg></span></label><label class="danpus-request-check surat-tindakan-check"><input type="checkbox" name="tindakan[]" value="INGATKAN"><span class="check-label">INGATKAN</span><span class="check-mark"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><path d="M5 13l4 4L19 7"/></svg></span></label><label class="danpus-request-check surat-tindakan-check"><input type="checkbox" name="tindakan[]" value="SARAN"><span class="check-label">SARAN</span><span class="check-mark"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><path d="M5 13l4 4L19 7"/></svg></span></label><label class="danpus-request-check surat-tindakan-check"><input type="checkbox" name="tindakan[]" value="MENGHADAP"><span class="check-label">MENGHADAP</span><span class="check-mark"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><path d="M5 13l4 4L19 7"/></svg></span></label><label class="danpus-request-check surat-tindakan-check"><input type="checkbox" name="tindakan[]" value="LAPORKAN HASILNYA"><span class="check-label">LAPORKAN HASILNYA</span><span class="check-mark"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><path d="M5 13l4 4L19 7"/></svg></span></label><label class="danpus-request-check surat-tindakan-check"><input type="checkbox" name="tindakan[]" value="IKUTI PERKEMBANGANNYA"><span class="check-label">IKUTI PERKEMBANGANNYA</span><span class="check-mark"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><path d="M5 13l4 4L19 7"/></svg></span></label><label class="danpus-request-check surat-tindakan-check"><input type="checkbox" name="tindakan[]" value="SIAPKAN"><span class="check-label">SIAPKAN</span><span class="check-mark"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><path d="M5 13l4 4L19 7"/></svg></span></label><label class="danpus-request-check surat-tindakan-check"><input type="checkbox" name="tindakan[]" value="SELESAIKAN"><span class="check-label">SELESAIKAN</span><span class="check-mark"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><path d="M5 13l4 4L19 7"/></svg></span></label><label class="danpus-request-check surat-tindakan-check"><input type="checkbox" name="tindakan[]" value="INFOKAN"><span class="check-label">INFOKAN</span><span class="check-mark"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><path d="M5 13l4 4L19 7"/></svg></span></label><label class="danpus-request-check surat-tindakan-check"><input type="checkbox" name="tindakan[]" value="ARSIP"><span class="check-label">ARSIP</span><span class="check-mark"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><path d="M5 13l4 4L19 7"/></svg></span></label><label class="danpus-request-check surat-tindakan-check"><input type="checkbox" name="tindakan[]" value="UDL"><span class="check-label">UDL</span><span class="check-mark"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><path d="M5 13l4 4L19 7"/></svg></span></label><label class="danpus-request-check surat-tindakan-check"><input type="checkbox" name="tindakan[]" value="UDK"><span class="check-label">UDK</span><span class="check-mark"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><path d="M5 13l4 4L19 7"/></svg></span></label><label class="danpus-request-check surat-tindakan-check"><input type="checkbox" name="tindakan[]" value="UMP"><span class="check-label">UMP</span><span class="check-mark"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><path d="M5 13l4 4L19 7"/></svg></span></label></div><span class="kirim-laporan-error" id="suratTindakanError" style="display:none"></span></div><div class="form-field full" id="suratDeskripsiField"><label for="surat_deskripsi">Isi Ringkasan Surat (opsional)</label><textarea id="surat_deskripsi" name="deskripsi" placeholder="Tuliskan isi surat yang ingin disampaikan..."></textarea></div></div><div class="surat-form-col surat-form-col-lampiran"><div class="surat-section-label">Lampiran</div><div class="surat-lampiran-zone" id="suratLampiranZone"><input id="surat_lampiran" type="file" class="surat-lampiran-zone-input" name="lampiran" data-file-picker-ready="1"><div class="surat-lampiran-zone-prompt"><span class="surat-lampiran-zone-icon"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M8 17l4-4 4 4"></path><path d="M12 13v9"></path><path d="M20.39 18.39A5 5 0 0 0 18 9h-1.26A8 8 0 1 0 3 16.3"></path></svg></span><span class="surat-lampiran-zone-text"><span class="surat-lampiran-zone-text-main">Klik untuk pilih file atau drag &amp; drop</span><span class="surat-lampiran-zone-text-sub">Maksimal 10 MB</span></span></div></div><div class="lampiran-file-list" id="suratLampiranFileList"><div class="lampiran-file-list-empty" id="suratLampiranFileListEmpty">Belum ada file dipilih</div></div><div class="surat-lampiran-preview" id="suratLampiranPreview" hidden></div><span class="kirim-laporan-error" id="suratLampiranError" style="display:none"></span></div></div><div class="modal-actions"><button type="button" class="btn" id="kirimSuratCancel">Batal</button><button class="btn btn-primary" type="submit" id="kirimSuratSubmitBtn">Buat Surat</button></div></form></div></div><div class="confirm-overlay" id="konfirmasiBuatSuratOverlay"><div class="confirm-box" role="alertdialog" aria-modal="true" aria-labelledby="konfirmasiBuatSuratTitle"><div class="confirm-icon" style="background:var(--gold-dim);color:var(--gold-bright)"><svg viewBox="0 0 24 24" stroke-linecap="round" stroke-linejoin="round" fill="none" stroke-width="1.9"><rect x="6" y="4" width="12" height="17" rx="2"></rect><path d="M9 4h6"></path><path d="M9 10h6"></path><path d="M9 14h6"></path><path d="M9 18h3"></path></svg></div><h3 id="konfirmasiBuatSuratTitle">Kirim Surat Baru?</h3><p>Pastikan data yang kamu isi sudah benar. Surat yang sudah dikirim tidak dapat diedit lagi.</p><div class="confirm-actions"><button type="button" class="btn" id="konfirmasiBuatSuratBatal">Batal</button><button type="button" class="btn btn-primary" id="konfirmasiBuatSuratYa">Ya, Kirim</button></div></div></div>
{{-- Panel kalender & jam buat Deadline surat prioritas Kilat (modal Buat
     Surat Baru Danpus) -- SENGAJA di luar .report-modal-card, dipindah ke
     <body> pas dibuka lewat initDanpusDatePicker/initDanpusTimePicker (lihat
     komentar "ancestor ber-transform" di situ), jadi lokasinya di markup
     bebas. Backdrop pakai #danpusPickerBackdrop yang sama (singleton satu
     halaman) biar tidak duplikat elemen. --}}
<div class="danpus-calendar" id="suratDeadlineCalendar" data-min="{{ now()->format('Y-m-d') }}"><div class="danpus-calendar-head"><button type="button" class="danpus-calendar-nav" id="suratDeadlineCalendarPrev" aria-label="Bulan sebelumnya"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M15 18l-6-6 6-6"/></svg></button><span class="danpus-calendar-title" id="suratDeadlineCalendarTitle"></span><button type="button" class="danpus-calendar-nav" id="suratDeadlineCalendarNext" aria-label="Bulan berikutnya"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M9 18l6-6-6-6"/></svg></button></div><div class="danpus-calendar-weekdays"><span>M</span><span>S</span><span>S</span><span>R</span><span>K</span><span>J</span><span>S</span></div><div class="danpus-calendar-days" id="suratDeadlineCalendarDays"></div><div class="danpus-picker-actions"><button type="button" class="danpus-picker-close" id="suratDeadlineCalendarClose">Tutup</button><button type="button" class="danpus-picker-confirm" id="suratDeadlineCalendarConfirm" disabled>Konfirmasi</button></div></div><div class="danpus-clock-panel" id="suratDeadlineClock"><div class="danpus-wheel-row"><div class="danpus-wheel-highlight"></div><div class="danpus-wheel" id="suratDeadlineWheelHour"></div><span class="danpus-wheel-colon">:</span><div class="danpus-wheel" id="suratDeadlineWheelMinute"></div></div><div class="danpus-picker-actions"><button type="button" class="danpus-picker-close" id="suratDeadlineClockClose">Tutup</button><button type="button" class="danpus-picker-confirm" id="suratDeadlineClockConfirm">Konfirmasi</button></div></div>
@include('siberad.dashboards.partials.surat-detail-modal')
@include('siberad.dashboards.partials.surat-teruskan-modal')
...
<div class="pimp-page"><section id="dashboard" class="tab-panel active"><div class="pimp-hero"><div class="pimp-eyebrow">{{ $pengaturan->namaSistem() }} // {{ $satuan->kode }}</div><h1>{{ $satuan->nama }}</h1><p>{{ now()->translatedFormat('l, d F Y') }}</p>@include('siberad.dashboards.partials.dash-hero-deco', ['icon' => 'star'])</div>@php
...
(function(){
  ...
  document.querySelectorAll('.logout-form').forEach(form=>form.addEventListener('submit',e=>{if(!window.confirm('Keluar dari akun {{ $pengaturan?->namaSistem() ?? "SIBERAD" }}?'))e.preventDefault()}));

  window.getLampiranExt=function(x){var nama=x.nama||'Lampiran';var m=nama.match(/\.([a-z0-9]+)$/i);if(m)return m[1].toLowerCase();try{var p=new URL(x.url,window.location.href).pathname;var m2=p.match(/\.([a-z0-9]+)$/i);return m2?m2[1].toLowerCase():''}catch(e){return ''}};
  window.getLampiranTypeInfo=function(ext){var IMG=['jpg','jpeg','png','gif','webp','bmp','svg'];if(IMG.indexOf(ext)!==-1)return{c:'#0ea5b7',label:'Gambar',glyph:'img'};var map={pdf:{c:'#ea4335',label:'PDF Document',glyph:'pdf'},doc:{c:'#1f6feb',label:'Microsoft Word',glyph:'W'},docx:{c:'#1f6feb',label:'Microsoft Word',glyph:'W'},xls:{c:'#1da55f',label:'Microsoft Excel',glyph:'X'},xlsx:{c:'#1da55f',label:'Microsoft Excel',glyph:'X'},csv:{c:'#1da55f',label:'Spreadsheet CSV',glyph:'X'},ppt:{c:'#ed6c0e',label:'Microsoft PowerPoint',glyph:'P'},pptx:{c:'#ed6c0e',label:'Microsoft PowerPoint',glyph:'P'},zip:{c:'#8a63d2',label:'Arsip ZIP',glyph:'zip'},rar:{c:'#8a63d2',label:'Arsip RAR',glyph:'zip'},txt:{c:'#6b7280',label:'Dokumen Teks',glyph:'txt'}};return map[ext]||{c:'#6b7280',label:ext?ext.toUpperCase()+' File':'File',glyph:'file'}};
  window.getLampiranIconSvg=function(ext){var info=window.getLampiranTypeInfo(ext);var badge='<rect x="1" y="1" width="22" height="22" rx="5" fill="'+info.c+'"></rect>';var page='<path d="M6.2 4.6c0-.4.3-.7.7-.7h5l2.3 2.3v9.2c0 .4-.3.7-.7.7H6.9c-.4 0-.7-.3-.7-.7Z" fill="#fff" fill-opacity=".2"></path><path d="M11.9 3.9v2.3h2.3" fill="none" stroke="#fff" stroke-opacity=".3" stroke-width=".9"></path>';var inner;if(info.glyph==='img'){inner='<circle cx="8.3" cy="9.3" r="1.7" fill="#fff"></circle><path d="M4 17.5l4.6-5.2 3.4 3.9 2.6-3 5.4 6.3H4z" fill="#fff" fill-opacity=".95"></path>';return '<svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">'+badge+inner+'</svg>'}if(info.glyph==='pdf'){inner='<text x="6" y="16" font-size="8.4" font-weight="800" font-family="Arial, sans-serif" fill="#fff">PDF</text>';return '<svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">'+badge+page+inner+'</svg>'}if(info.glyph==='zip'){inner='<rect x="10.6" y="4.5" width="2.8" height="12.5" fill="#fff" fill-opacity=".9"></rect><circle cx="12" cy="16.5" r="2.2" fill="#fff"></circle>';return '<svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">'+badge+page+inner+'</svg>'}if(info.glyph==='txt'){inner='<path d="M7 8h10M7 12h10M7 16h6.5" stroke="#fff" stroke-width="1.6" stroke-linecap="round"></path>';return '<svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">'+badge+page+inner+'</svg>'}if(info.glyph==='file'){inner='<path d="M7.5 8h9M7.5 12h9M7.5 16h5.5" stroke="#fff" stroke-width="1.6" stroke-linecap="round" stroke-opacity=".85"></path>';return '<svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">'+badge+page+inner+'</svg>'}var letter='<text x="6.2" y="16.6" font-size="11.5" font-weight="800" font-family="Arial, sans-serif" fill="#fff">'+info.glyph+'</text>';var accent='';if(info.glyph==='W'){accent='<path d="M14.9 10h3M14.9 12.3h3M14.9 14.6h2.1" stroke="#fff" stroke-opacity=".9" stroke-width="1.1" stroke-linecap="round"></path>'}else if(info.glyph==='X'){accent='<rect x="14.6" y="9.6" width="2" height="2" fill="#fff" fill-opacity=".9"></rect><rect x="17.2" y="9.6" width="2" height="2" fill="#fff" fill-opacity=".9"></rect><rect x="14.6" y="12.1" width="2" height="2" fill="#fff" fill-opacity=".9"></rect><rect x="17.2" y="12.1" width="2" height="2" fill="#fff" fill-opacity=".9"></rect>'}else if(info.glyph==='P'){accent='<circle cx="16.6" cy="11.6" r="2.4" fill="none" stroke="#fff" stroke-opacity=".9" stroke-width="1.1"></circle><path d="M16.6 9.2v2.4h2.4" fill="none" stroke="#fff" stroke-opacity=".9" stroke-width="1.1" stroke-linecap="round"></path>'}return '<svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">'+badge+page+letter+accent+'</svg>'};
  window.renderLampiranPreview=function(list,container){var IMG_EXT=['jpg','jpeg','png','gif','webp','bmp','svg'];function getExt(x){var nama=x.nama||'Lampiran';var m=nama.match(/\.([a-z0-9]+)$/i);if(m)return m[1].toLowerCase();try{var p=new URL(x.url,window.location.href).pathname;var m2=p.match(/\.([a-z0-9]+)$/i);return m2?m2[1].toLowerCase():''}catch(e){return ''}}function renderRow(x,ext){var nama=x.nama||'Lampiran';var item=document.createElement('div');item.className='lampiran-preview-file';item.innerHTML='<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"></path><path d="M14 2v6h6"></path></svg><div class="lampiran-preview-file-info"><span class="lampiran-preview-file-name"></span><span class="lampiran-preview-file-hint"></span></div>';item.querySelector('.lampiran-preview-file-name').textContent=nama;item.querySelector('.lampiran-preview-file-hint').textContent=IMG_EXT.indexOf(ext)!==-1?'Gambar':(ext==='pdf'?'Dokumen PDF':'Format ini tidak bisa ditampilkan langsung');var linkRow=document.createElement('a');linkRow.href=x.url;linkRow.target='_blank';linkRow.rel='noopener';linkRow.textContent='Buka file';item.appendChild(linkRow);container.appendChild(item)}list.forEach(function(x){renderRow(x,getExt(x))})};window.openReportDetail=function(button){const modal=document.getElementById('reportDetailModal');document.getElementById('detailPengirim').textContent=button.dataset.pengirim||'-';document.getElementById('detailTujuan').textContent=button.dataset.tujuan||'-';document.getElementById('detailPerihal').textContent=button.dataset.perihal||'-';document.getElementById('detailPrioritas').textContent=button.dataset.prioritas||'-';document.getElementById('detailProgres').textContent=button.dataset.progres?button.dataset.progres+'%':'-';const progresWrap=document.getElementById('detailProgresWrap');if(progresWrap)progresWrap.style.display=button.dataset.kendalaReport==='1'?'none':'block';document.getElementById('detailProyek').textContent=button.dataset.proyek||'-';document.getElementById('detailTanggal').textContent=button.dataset.tanggal||'-';document.getElementById('detailDeskripsi').textContent=button.dataset.deskripsi||'-';const kendala=button.dataset.kendala||'',kendalaWrap=document.getElementById('detailKendalaWrap');if(kendala){document.getElementById('detailKendala').textContent=kendala;kendalaWrap.style.display='block'}else{kendalaWrap.style.display='none'}const wrap=document.getElementById('detailLampiranWrap');const linkBox=document.getElementById('detailLampiran');let daftarLampiran=[];try{daftarLampiran=button.dataset.lampiran?JSON.parse(button.dataset.lampiran):[]}catch(e){daftarLampiran=[]}linkBox.innerHTML='';linkBox.className='detail-value';if(daftarLampiran.length){if(button.dataset.kendalaReport==='1'){linkBox.classList.add('lampiran-detail-list');daftarLampiran.forEach(function(x,i){const ext=window.getLampiranExt(x);const info=window.getLampiranTypeInfo(ext);const card=document.createElement('div');card.className='lampiran-detail-card';card.style.cursor='pointer';card.title='Klik untuk melihat file';card.addEventListener('click',function(){window.open(x.url,'_blank','noopener')});const iconBox=document.createElement('div');iconBox.className='lampiran-detail-icon';iconBox.innerHTML=window.getLampiranIconSvg(ext);const infoBox=document.createElement('div');infoBox.className='lampiran-detail-info';const nameEl=document.createElement('span');nameEl.className='lampiran-detail-name';nameEl.textContent=x.nama||'Lampiran '+(i+1);const metaEl=document.createElement('span');metaEl.className='lampiran-detail-meta';metaEl.textContent=info.label+(x.ukuran?' · '+x.ukuran:'');infoBox.appendChild(nameEl);infoBox.appendChild(metaEl);const actions=document.createElement('div');actions.className='lampiran-detail-actions';const dl=document.createElement('a');dl.href=x.url;dl.setAttribute('download',x.nama||'');dl.className='lampiran-detail-dl';dl.title='Unduh';dl.setAttribute('aria-label','Unduh '+(x.nama||'lampiran'));dl.onclick=function(e){e.stopPropagation()};dl.innerHTML='<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 3v12"></path><path d="m7 10 5 5 5-5"></path><path d="M5 21h14"></path></svg>';actions.appendChild(dl);card.appendChild(iconBox);card.appendChild(infoBox);card.appendChild(actions);linkBox.appendChild(card)})}else{daftarLampiran.forEach(function(x,i){const a=document.createElement('a');a.href=x.url;a.target='_blank';a.rel='noopener';a.className='lampiran-btn';a.innerHTML='<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"></path><path d="M14 2v6h6"></path></svg><span>'+(daftarLampiran.length>1?(x.nama||'Lihat Lampiran '+(i+1)):'Lihat Lampiran')+'</span>';linkBox.appendChild(a)})}wrap.style.display='block'}else{wrap.style.display='none'}const actionsEl=document.getElementById('detailActions')||modal?.querySelector('.modal-actions');if(actionsEl){if(button.dataset.suratMasuk==='1'){actionsEl.innerHTML='';const note=document.createElement('span');note.style.cssText='font-size:11px;color:var(--p-muted);margin-right:auto';note.textContent=button.dataset.readonlyText||'-';actionsEl.appendChild(note);if(button.dataset.konfirmasiAction){const kbtn=document.createElement('button');kbtn.type='button';kbtn.className='detail-btn';kbtn.style.cssText='background:var(--clr-accent,#1a7a4a);color:#fff;border-color:var(--clr-accent,#1a7a4a)';kbtn.textContent='Konfirmasi';kbtn.onclick=function(){if(typeof window.bukaKonfirmasiSurat==='function')window.bukaKonfirmasiSurat(button.dataset.konfirmasiAction,button.dataset.konfirmasiToken,button.dataset.konfirmasiPengirim)};actionsEl.appendChild(kbtn)}}else if(button.dataset.pimpinanReview==='1'){const lid=button.dataset.laporanId||'';actionsEl.innerHTML='<div class="action-row"><button type="button" class="btn reject" onclick="bukaTolakLaporanPimpinan(\''+lid+'\')">Tolak</button><button type="button" class="btn approve" onclick="bukaTerimaLaporanPimpinan(\''+lid+'\')">Terima</button></div>'}else if(button.dataset.deletable==='1'){actionsEl.innerHTML='<div class="action-row"><button type="button" class="reject" onclick="bukaHapusRiwayatPimpinan(\''+(button.dataset.id||'')+'\')">Hapus dari Riwayat</button></div>'}else if(button.dataset.kendalaReport==='1'){actionsEl.innerHTML=''}else if(button.dataset.readonly==='1'){actionsEl.innerHTML='<span class="report-detail-note" style="margin-right:auto;font-size:11px;color:var(--p-muted);">'+(button.dataset.readonlyText||'Mode pimpinan: aktivitas Satlak bersifat view-only.')+'</span>'}else{actionsEl.innerHTML='<span class="readonly-note">Mode pimpinan: aktivitas Satlak bersifat view-only.</span>'}}modal?.classList.add('open')};
  ...
  document.getElementById('kirimSuratOpen')?.addEventListener('click',()=>{const m=document.getElementById('kirimSuratModal');if(!m)return;void m.offsetHeight;m.classList.add('open');m.style.pointerEvents='auto';});
  document.getElementById('kirimSuratClose')?.addEventListener('click',()=>{const m=document.getElementById('kirimSuratModal');m?.classList.remove('open');if(m)m.style.pointerEvents='';});
  document.getElementById('kirimSuratCancel')?.addEventListener('click',()=>{const m=document.getElementById('kirimSuratModal');m?.classList.remove('open');if(m)m.style.pointerEvents='';});
  document.addEventListener('keydown',e=>{if(e.key==='Escape'&&document.getElementById('kirimSuratModal')?.classList.contains('open')){const m=document.getElementById('kirimSuratModal');m.classList.remove('open');m.style.pointerEvents='';}});
  (function(){
  ...
  const suratMessages={
    ...
    disposisi:'Disposisi wajib dipilih.',
    deskripsi:'Isi ringkasan surat wajib diisi.',
    // Proxy tanggal/jam deadline nggak punya attribute `name` (nilai
    // aslinya digabung ke hidden `deadline_at` pas submit, lihat picker
    // init di bawah) -- makanya dikunci ke lookup `id` juga.
    suratDeadlineDateProxy:'Tanggal deadline wajib diisi.',
    suratDeadlineTimeProxy:'Jam deadline wajib diisi.'
  };
  form.querySelectorAll('select[required],input[required],textarea[required]').forEach(function(input){
    ...
    input.addEventListener('invalid',function(e){
      ...
      msg.textContent=suratMessages[input.name]||suratMessages[input.id]||'Kolom ini wajib diisi.';
  ...
  (function(){
  // Jenis Surat (Biasa/Kilat/Rahasia): klik ulang opsi yang sudah
  // terpilih membatalkan pilihannya. Radio-nya sendiri diberi
  // pointer-events:none lewat CSS .priority-option (yang keklik itu
  // <label>-nya) dan checkedness-nya sudah keburu di-set TRUE oleh browser
  // sebelum 'click' nyampe ke sini -- jadi nggak bisa pakai mousedown/nilai
  // checked saat ini buat tau "ini klik pertama atau klik ulang". Makanya
  // dilacak manual pakai nilai yang lagi kepilih di luar elemennya.
  const radios=document.querySelectorAll('#kirimSuratModal input[name="prioritas"]');
  let prioritasTerpilih=null;
  radios.forEach(function(r){ if(r.checked) prioritasTerpilih=r.value; });
  radios.forEach(function(r){
    r.addEventListener('click',function(){
      if(r.value===prioritasTerpilih){
        r.checked=false;
        prioritasTerpilih=null;
        r.dispatchEvent(new Event('change',{bubbles:true}));
      } else {
        prioritasTerpilih=r.value;
      }
    });
  });

  // Isi Ringkasan Surat sekarang SELALU opsional (gak ada attribute
  // required di HTML-nya lagi) -- khusus Rahasia, field-nya masih
  // disembunyikan aja karena isinya toh nggak bakal ditampilkan ke
  // penerima (lihat LaporanSurat::ringkasanUntuk()).
  const deskripsiField=document.getElementById('suratDeskripsiField');
  const deskripsiInput=document.getElementById('surat_deskripsi');
  function syncDeskripsiUntukRahasia(){
    if(!deskripsiField||!deskripsiInput)return;
    const rahasia=prioritasTerpilih==='Rahasia';
    deskripsiField.style.display=rahasia?'none':'';
    if(rahasia)deskripsiInput.classList.remove('field-invalid');
  }
  radios.forEach(function(r){ r.addEventListener('change',syncDeskripsiUntukRahasia); });
  syncDeskripsiUntukRahasia();
  ...
  function syncDeadlineUntukKilat(){
  ...
  radios.forEach(function(r){ r.addEventListener('change',syncDeadlineUntukKilat); });
  syncDeadlineUntukKilat();
  })();
  (function(){
  const form=document.getElementById('kirimSuratForm');
  const grid=document.getElementById('suratTindakanGrid');
  const errEl=document.getElementById('suratTindakanError');
  if(!form||!grid||!errEl)return;
  function checkedCount(){return grid.querySelectorAll('input[name="tindakan[]"]:checked').length}
  function showError(){
    grid.classList.add('field-invalid');
    errEl.textContent='Tindakan wajib dipilih (minimal satu).';
  ...
  (function(){
    ...
    // ---- Deadline khusus prioritas "Kilat" di modal Buat Surat Baru
    // (Danpus) -- widget sama persis (reuse initDanpusDatePicker/
    // initDanpusTimePicker generik di atas), cuma bedanya field ini
    // opsional-tampil (disembunyikan/dimunculkan lewat toggle Prioritas
    // Surat, lihat IIFE-nya). Diekspos ke window supaya toggle Prioritas
    // itu -- IIFE TERPISAH yang jalan lebih dulu -- bisa manggil
    // setPicked(null) buat nge-reset pilihan begitu prioritas dipindah
    // menjauh dari Kilat. ----
    window.suratDeadlineDatePicker=initDanpusDatePicker({input:'suratDeadlineDateInput',proxy:'suratDeadlineDateProxy',picker:'suratDeadlineDatePickerBox',panel:'suratDeadlineCalendar',days:'suratDeadlineCalendarDays',title:'suratDeadlineCalendarTitle',confirm:'suratDeadlineCalendarConfirm',prev:'suratDeadlineCalendarPrev',next:'suratDeadlineCalendarNext',close:'suratDeadlineCalendarClose',backdrop:'danpusPickerBackdrop'});
    window.suratDeadlineTimePicker=initDanpusTimePicker({input:'suratDeadlineTimeInput',proxy:'suratDeadlineTimeProxy',picker:'suratDeadlineTimePickerBox',panel:'suratDeadlineClock',hourWheel:'suratDeadlineWheelHour',minuteWheel:'suratDeadlineWheelMinute',confirm:'suratDeadlineClockConfirm',close:'suratDeadlineClockClose',backdrop:'danpusPickerBackdrop'});
    const suratDeadlineHidden=document.getElementById('suratDeadlineHidden');
    document.getElementById('kirimSuratForm')?.addEventListener('submit',()=>{
      const d=window.suratDeadlineDatePicker?.getPicked(),t=window.suratDeadlineTimePicker?.getPicked();
      if(suratDeadlineHidden)suratDeadlineHidden.value=(d&&t)?(d+'T'+t):'';
    });
  ...
  function makeStatusDonut(){
    ...
    const chart=new Chart(el,{
      ...
      options:{cutout:'70%',responsive:true,maintainAspectRatio:false,layout:{padding:24},
        ...
        // di-refresh ulang (bug yang dilaporkan user).
