...
(function(){
    ...
    function syncEmptyState(){var section=document.getElementById('permintaan-laporan');var list=section&&section.querySelector('.deadline-sender-list');if(!section||!list)return;var hasItems=!!list.querySelector('.deadline-sender-item, [data-realtime-permintaan-id]');var emptyNode=null;var candidates=section.querySelectorAll('*');for(var i=0;i<candidates.length;i++){var el=candidates[i];if(el===list||list.contains(el))continue;var text=(el.textContent||'').replace(/\s+/g,' ').trim();if(text.indexOf('Belum ada permintaan laporan')!==-1&&text.indexOf('Tugas pelaporan dari Danpus/Wadan akan muncul di sini.')!==-1){emptyNode=el;break;}}if(!emptyNode)return;var target=emptyNode;while(target.parentElement&&target.parentElement!==section&&((target.parentElement.textContent||'').indexOf('Tugas pelaporan dari Danpus/Wadan akan muncul di sini.')!==-1)){target=target.parentElement;}target.style.display=hasItems?'none':'';target.setAttribute('aria-hidden',hasItems?'true':'false');}
    function insertItems(itemsHtml){var list=document.querySelector(listSelector);if(!list||!itemsHtml)return false;var temp=document.createElement('div');temp.innerHTML=itemsHtml;var items=Array.prototype.slice.call(temp.children);if(!items.length)return false;var existing={};list.querySelectorAll('[data-realtime-permintaan-id], .use-permintaan[data-request-id]').forEach(function(el){var id=el.getAttribute('data-realtime-permintaan-id')||el.getAttribute('data-request-id');if(id)existing[id]=true;});
        ...
        oldCards.forEach(function(card){
            ...
            card.style.transition='transform .38s cubic-bezier(.4,0,.2,1)';
    ...
    function showList(animate){var section=document.getElementById('permintaan-laporan');var view=document.getElementById(detailViewId);if(!section)return;if(view){view.classList.remove('is-visible');window.setTimeout(function(){view.style.display='none';},220);}}
    function openDetail(item){var section=document.getElementById('permintaan-laporan');var view=ensureDetailView();if(!section||!view)return;var title=item.querySelector('.deadline-sender-title');var meta=item.querySelector('.deadline-sender-meta');var instruction=item.querySelector('.deadline-sender-instruction');var pill=item.querySelector('.deadline-pill');var actionForm=item.querySelector('form[action*="/permintaan-laporan/"]');var target=item.querySelector('.use-permintaan');var sender=(meta&&meta.textContent||'').replace(/^Dari\s*/i,'').split('·')[0].trim();var deadline=(meta&&meta.textContent||'').split('· Deadline').slice(1).join('· Deadline').trim();var category=item.getAttribute('data-kategori')||(target&&target.getAttribute('data-kategori'))||'-';var priority=item.getAttribute('data-prioritas')||(target&&target.getAttribute('data-prioritas'))||'-';var instr=target&&target.getAttribute('data-instruksi')||((instruction&&instruction.textContent)||'-');var formAction=actionForm&&actionForm.getAttribute('action')||'';if(!formAction){var id=item.getAttribute('data-realtime-permintaan-id')||'';if(id)formAction='{{ url('/permintaan-laporan') }}/'+id+'/mulai';}view.querySelector('#permintaanDetailPengirim').textContent=sender||'Pimpinan';
        ...
        view.querySelector('#permintaanDetailInstruksi').textContent=instr||'-';var confirmForm=view.querySelector('#permintaanDetailConfirmForm');confirmForm.setAttribute('action',formAction);view.querySelector('#permintaanDetailConfirmBtn').disabled=!formAction;
    ...
    function bindDetailButtons(){var list=document.querySelector(listSelector);if(!list)return;list.querySelectorAll('.permintaan-lihat-detail-btn').forEach(function(button){var item=button.closest('.deadline-sender-item');if(!item||button.dataset.detailBound==='1')return;button.dataset.detailBound='1';button.addEventListener('click',function(e){e.preventDefault();openDetail(item);});});}
    window.siberadBindPermintaanDetailButtons=bindDetailButtons;
    function poll(initial){if(polling)return;polling=true;var since=initial?0:lastSeen;fetch(endpoint+'?since='+encodeURIComponent(since),{method:'GET',credentials:'same-origin',cache:'no-store',headers:{'Accept':'application/json','X-Requested-With':'XMLHttpRequest'}}).then(function(response){if(response.status===401){if(window.siberadTampilkanSesiBerakhir)window.siberadTampilkanSesiBerakhir();return null;}if(response.status===419)return null;if(!response.ok)throw new Error('Realtime request failed');return response.json();}).then(function(data){if(!data)return;syncIncomingReportCount(data);var inserted=insertItems(data.items_html);bindDetailButtons();syncEmptyState();window.siberadRebindPermintaanActions&&window.siberadRebindPermintaanActions();if(!initial&&inserted&&window.siberadShowToast)window.siberadShowToast('success','Permintaan laporan baru masuk.');lastSeen=Math.max(lastSeen,parseInt(data.latest_id||0,10));if(initial)lastSeen=Math.max(lastSeen,existingLatestId());initialPoll=false;}).catch(function(){}).finally(function(){polling=false;});}
    function start(){if(!document.querySelector(listSelector))return;bindDetailButtons();syncEmptyState();poll(true);window.setInterval(function(){poll(false);},3000);}
