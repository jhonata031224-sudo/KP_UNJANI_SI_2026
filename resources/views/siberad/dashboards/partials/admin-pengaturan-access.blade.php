...
.admin-access-card{width:400px;max-width:calc(100vw - 36px);box-sizing:border-box;background:#fff;color:#17212b;border:1px solid #e2e8f0;border-radius:18px;box-shadow:0 28px 72px rgba(0,0,0,.32);padding:24px 24px 22px;position:relative;transform:translateY(10px) scale(.985);transition:transform .2s ease}
...
.admin-access-icon{width:58px;height:58px;border-radius:15px;background:#fff3dc;color:#EEB341;display:flex;align-items:center;justify-content:center;margin-bottom:16px}
...
.admin-access-field{margin-top:14px}
...
.admin-access-input:focus{border-color:#EEB341;box-shadow:0 0 0 3px rgba(201,122,0,.12);background:#fff}
...
.admin-access-refresh:hover{border-color:#EEB341;color:#EEB341;background:#fffaf1}
...
.admin-access-captcha-input{min-width:0;padding-left:12px;padding-right:10px;font-size:14px}
.admin-access-captcha-input::placeholder{white-space:nowrap;overflow:hidden;text-overflow:clip}
...
.admin-access-actions{display:flex;align-items:center;justify-content:flex-end;gap:10px;margin-top:20px}
...
.admin-access-btn.submit{border:1px solid #EEB341;background:#EEB341;color:#111827;min-width:176px;box-shadow:0 5px 14px rgba(201,122,0,.18)}
...
<p class="admin-access-desc">Menu ini berisi pengaturan yang dapat mengubah tampilan landing page. Masukkan password dan captcha untuk melanjutkan.</p>
<div class="admin-access-field"><label class="admin-access-label" for="adminAccessPassword">Password</label><div class="admin-access-input-wrap"><input id="adminAccessPassword" name="pengaturan_umum_access_password" class="admin-access-input" type="password" autocomplete="new-password" placeholder="Masukkan password"><button type="button" class="admin-access-password-toggle" id="adminAccessToggle" aria-label="Tampilkan password"><svg viewBox="0 0 24 24"><path d="M2 12s3.5-6 10-6 10 6 10 6-3.5 6-10 6S2 12 2 12Z"/><circle cx="12" cy="12" r="2.5"/></svg></button></div></div>
<div class="admin-access-field"><label class="admin-access-label" for="adminAccessCaptcha">Captcha</label><div class="admin-access-captcha-row"><img id="adminAccessCaptchaImage" class="admin-access-captcha-image" alt="Captcha verifikasi akses"><button type="button" class="admin-access-refresh" id="adminAccessRefresh" aria-label="Muat ulang captcha"><svg viewBox="0 0 24 24"><path d="M20 11a8 8 0 0 0-14.9-3.8L3 10"/><path d="M3 4v6h6"/><path d="M4 13a8 8 0 0 0 14.9 3.8L21 14"/><path d="M21 20v-6h-6"/></svg></button><input id="adminAccessCaptcha" name="pengaturan_umum_access_captcha" class="admin-access-input admin-access-captcha-input" type="text" inputmode="text" maxlength="5" autocomplete="off" placeholder="Masukan captcha"></div></div>
...
var modal=document.getElementById('adminPengaturanAccessModal'),pass=document.getElementById('adminAccessPassword'),captcha=document.getElementById('adminAccessCaptcha'),img=document.getElementById('adminAccessCaptchaImage'),refresh=document.getElementById('adminAccessRefresh'),error=document.getElementById('adminAccessError'),submit=document.getElementById('adminAccessSubmit'),lockStyle=null;
var csrfMeta=document.querySelector('meta[name="csrf-token"]');
var csrfToken=csrfMeta?csrfMeta.content:@json(csrf_token());
if(!modal)return;
function syncCsrfToken(token){if(!token)return;csrfToken=token;if(csrfMeta)csrfMeta.content=token;document.querySelectorAll('input[name="_token"]').forEach(function(inp){inp.value=token})}
...
function revoke(){if(!granted)return;granted=false;lockPanel();fetch(verifyUrl,{method:'POST',headers:{'Content-Type':'application/json','Accept':'application/json','X-CSRF-TOKEN':csrfToken},body:JSON.stringify({action:'revoke'})}).catch(function(){})}
function verify(){var pw=pass.value.trim(),cp=captcha.value.trim();if(!pw||cp.length!==5){setError('Masukkan password dan 5 karakter captcha.');return}submit.disabled=true;setError('');fetch(verifyUrl,{method:'POST',headers:{'Content-Type':'application/json','Accept':'application/json','X-CSRF-TOKEN':csrfToken},body:JSON.stringify({action:'verify',password:pw,captcha:cp})}).then(function(r){return r.json().catch(function(){return {ok:false,message:'Respons server tidak valid.'}}).then(function(data){if(!r.ok||!data.ok)throw new Error(data.message||'Verifikasi gagal.');syncCsrfToken(data.csrf_token);granted=true;unlockPanel();closeModal();activateSettingTab()})}).catch(function(err){setError(err.message||'Verifikasi gagal.');refreshCaptcha()}).finally(function(){submit.disabled=false})}
