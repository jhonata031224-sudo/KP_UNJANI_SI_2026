...
<div class="surat-file-card" data-surat-id="{{ $s->id }}" data-created-at="{{ $s->created_at->timestamp }}" data-search="{{ strtolower($s->perihal.' '.$tujuanChainDisplay.' '.($s->tujuanSatuan->kode ?? '')) }}" data-prioritas="{{ $s->prioritas }}">
    ...
    <div><div class="surat-file-card-dari-label">Kepada</div><div class="surat-file-card-dari-value surat-tujuan-flow surat-tujuan-flow-sm">@foreach($tujuanHops as $hop)<div class="surat-tujuan-flow-row"><span class="surat-tujuan-flow-dot{{ $loop->last ? ' is-final' : '' }}"></span><span class="surat-tujuan-flow-step{{ $loop->last ? ' is-final' : '' }}">{{ $hop['nama'] }}</span></div>@if(!$loop->last)<span class="surat-tujuan-flow-connector"></span>@endif@endforeach</div></div>
