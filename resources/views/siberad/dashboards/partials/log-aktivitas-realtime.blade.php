...
(function () {
    ...
    function openProgressDetail(details, value) {
        if (!details) return;
        const rows = [...details.querySelectorAll('tr[data-progres]')];
        const row = rows.reverse().find(r => String(r.dataset.progres) === String(value));
        const button = row?.querySelector('.detail-btn');
        if (button) button.click();
    }
    function createProgressItem(value, latestValue, oldSet, details) {
        ...
        const valueText=document.createElement('strong'); valueText.textContent=`${value}%`;
        const label=document.createElement('small'); label.textContent=String(value)===String(latestValue)?'Terbaru':'Tercatat';
        ...
        detail.addEventListener('click',event=>{ event.preventDefault(); event.stopPropagation(); openProgressDetail(details,value); });
    ...
    function renderHistory(details, incomingValues) {
        ...
        for(let start=0,rowIndex=0;start<values.length;start+=columns,rowIndex++){
            ...
            values.slice(start,start+columns).forEach(value=>row.appendChild(createProgressItem(value,latestValue,oldSet,details)));
