...
.deadline-task-track{display:flex;flex-wrap:wrap;row-gap:8px;margin:8px 0;justify-content:flex-end}
.deadline-task-step{position:relative;display:flex;align-items:center;gap:6px;border:0;cursor:pointer;padding:7px 16px 7px 20px;margin-right:-11px;font:inherit;font-size:10.5px;font-weight:700;white-space:nowrap;background:var(--p-surface-2,var(--panel-alt));color:var(--p-muted,var(--text-muted));clip-path:polygon(0 50%,13px 0,100% 0,calc(100% - 13px) 50%,100% 100%,13px 100%);transition:background .15s ease,color .15s ease,filter .15s ease}
.deadline-task-step:first-child{clip-path:polygon(0 0,100% 0,calc(100% - 13px) 50%,100% 100%,0 100%);padding-left:14px}
.deadline-task-step:last-child{margin-right:0;clip-path:polygon(0 50%,13px 0,100% 0,100% 100%,13px 100%);padding-right:14px}
...
.deadline-task-step:disabled{cursor:wait;opacity:.7}
...
.deadline-task-step.done{background:#efe6d2;color:#8a7245}
.deadline-task-step.done .deadline-task-num{background:var(--green,#16834b);color:#fff}
@media(max-width:640px){.deadline-task-step{clip-path:none!important;margin-right:0;border-radius:8px;padding:7px 12px}}
</style>
