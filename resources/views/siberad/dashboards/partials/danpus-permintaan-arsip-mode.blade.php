...
#permintaan-laporan .danpus-archive-head-cell{justify-content:flex-start;gap:8px;transform:translateX(6px)}
#permintaan-laporan .danpus-archive-row-cell{justify-content:flex-start;gap:9px;width:100%;min-width:0;transform:translateX(6px)}
...
/* Spacer transparan agar baris non-eligible punya geometri identik dengan baris eligible. */
...
/* Kunci geometri tabel saat arsip aktif: tidak ada reflow/scroll jump, hanya kontrol yang muncul. */
...
#permintaan-laporan.danpus-archive-mode .request-table tbody tr td{height:64px}
...
function initDanpusArchiveMode(){
 ...
 const toggleLabel=archiveButton.querySelector('.danpus-archive-toggle-label'); const countBadge=document.createElement('span'); countBadge.className='danpus-archive-count';countBadge.id='danpusArchiveSelectedCount';countBadge.setAttribute('aria-live','polite');countBadge.textContent='0';actions.insertBefore(countBadge,archiveButton);
 ...
 function setMode(next){active=!!next;panel.classList.toggle('danpus-archive-mode',active);archiveButton.classList.toggle('is-active',active);archiveButton.setAttribute('aria-pressed',active?'true':'false');toggleLabel.textContent=active?'Selesai':'Arsip';if(!active)selectedIds.clear();syncRows()}
