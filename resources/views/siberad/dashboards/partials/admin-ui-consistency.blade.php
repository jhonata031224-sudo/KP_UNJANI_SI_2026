...
/* Tombol Tutup & Hapus Semua pada header notifikasi. */
...
#notifDropdown .siberad-notif-head-actions{display:flex;align-items:center;gap:12px;}
#notifDropdown .siberad-notif-hapus-semua-text{border:0;background:transparent;color:var(--red,#c83b3b);font:600 12px var(--body);padding:4px 0;cursor:pointer;white-space:nowrap;}
#notifDropdown .siberad-notif-hapus-semua-text:hover{text-decoration:underline;}
</style>
...
.main .content [data-tab-panel="backup"] .backup-upload-trigger{flex-shrink:0;font-weight:700!important;}
...
.siberad-backup-upload-actions{display:flex;justify-content:flex-end;gap:9px;margin-top:18px;}.siberad-backup-upload-actions .btn{font-weight:700!important;}
...
(function(){
  function addNotificationCloseText(){
    ...
    // Dibungkus satu wrapper flex biar "Hapus Semua" & "Tutup" NEMPEL
    // bersebelahan di kanan header, walau header-nya pakai
    // justify-content:space-between (title tetap di kiri).
    var actionsWrap=document.createElement('div');actionsWrap.className='siberad-notif-head-actions';
    var hapusSemua=document.createElement('button');hapusSemua.type='button';hapusSemua.className='siberad-notif-hapus-semua-text';hapusSemua.textContent='Hapus Semua';hapusSemua.setAttribute('aria-label','Hapus semua notifikasi');
    hapusSemua.addEventListener('click',function(e){e.preventDefault();e.stopPropagation();if(typeof window.siberadHapusSemuaNotifikasi==='function')window.siberadHapusSemuaNotifikasi();});
    ...
    actionsWrap.appendChild(hapusSemua);actionsWrap.appendChild(close);header.appendChild(actionsWrap);
  }
...
(function(){
  function initBackupUploadModal(){
    ...
    modal.innerHTML='<div class="siberad-backup-upload-card" role="dialog" aria-modal="true" aria-labelledby="siberadBackupUploadTitle">'+
      '<div class="siberad-backup-upload-head"><div><h3 id="siberadBackupUploadTitle">Unggah File Cadangan</h3><p>Pilih file backup yang ingin ditambahkan ke riwayat backup.</p></div><button type="button" class="siberad-backup-upload-close" aria-label="Tutup">×</button></div>'+ 
      ...
      '<div class="siberad-backup-upload-actions"><button type="button" class="btn btn-ghost backup-upload-cancel">Batal</button><button type="submit" class="btn btn-primary">Unggah</button></div></form></div>';
