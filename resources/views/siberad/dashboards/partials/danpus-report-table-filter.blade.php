<style>
/* Pimpinan: pencarian & filter untuk Laporan Masuk, Status Laporan, dan Permintaan Laporan. */
...
.rpt-filter-search svg{position:absolute;left:11px;top:50%;width:15px;height:15px;transform:translateY(-50%);color:var(--p-muted);pointer-events:none}
.rpt-filter-search input{box-sizing:border-box;width:100%;height:38px;border:1px solid var(--p-border);border-radius:9px;outline:0;background:var(--p-surface-2);color:var(--p-text);font:inherit;font-size:12px;padding:8px 11px 8px 34px}
.rpt-filter-search input:focus{border-color:var(--p-accent);box-shadow:0 0 0 3px rgba(201,122,0,.10)}
.rpt-filter-search input::placeholder{color:var(--p-muted)}
.rpt-filter-select{height:38px;border:1px solid var(--p-border);border-radius:9px;background-color:var(--p-surface-2);color:var(--p-text);font:inherit;font-size:11px;padding:0 28px 0 11px;outline:0;cursor:pointer;appearance:none;-webkit-appearance:none;background-image:url("data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 24 24' fill='none' stroke='%23888' stroke-width='2' stroke-linecap='round' stroke-linejoin='round'><path d='m6 9 6 6 6-6'/></svg>");background-repeat:no-repeat;background-position:right 9px center;background-size:13px}
.rpt-filter-select:focus{border-color:var(--p-accent);box-shadow:0 0 0 3px rgba(201,122,0,.10)}
.rpt-filter-count{font-size:10px;color:var(--p-muted);white-space:nowrap;margin-left:auto}
.rpt-filter-empty{padding:24px 12px;text-align:center;color:var(--p-muted);font-size:12px;border:1px dashed var(--p-border);border-radius:10px}
