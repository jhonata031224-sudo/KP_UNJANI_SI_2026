...
@media(max-width:980px){.report-grid{grid-template-columns:1fr 1fr}.pimp-kpis{grid-template-columns:repeat(2,1fr)}.report-layout{grid-template-columns:1fr}.monitor-grid{grid-template-columns:1fr 1fr}.satuan-terbaru-row{grid-template-columns:1fr}.monitor-kpi-row{grid-template-columns:repeat(2,1fr)}}@media(max-width:650px){.report-grid,.pimp-kpis,.form-grid,.monitor-grid,.detail-grid{grid-template-columns:1fr}.form-field.full,.detail-item.full{grid-column:auto}.monitor-kpi-row{grid-template-columns:repeat(2,1fr);gap:12px}.monitor-search{width:100%;flex:1 1 100%}.monitor-toolbar select{flex:1 1 calc(50% - 5px);min-width:0;max-width:none}}
...
@if($bisaKirimSurat)@php
  ...
  // Fondasi alur "Surat dari Satlak ke Urdal": khusus 4 Satlak (Kal, Sisos,
  // Dak, Duktek) tujuan utama Surat Keluar SELALU dikunci ke Urdal --
  // field-nya jadi read-only, bukan combobox bebas. Tembusan (CC) tetap
  // bebas pilih satuan lain, KECUALI Danpus & Wadan (validasi ganda juga
  // ada di LaporanSuratController::store()).
  $isSatlakPengirim    = in_array(strtoupper($satuan->kode ?? ''), \App\Models\Satuan::KODE_SATLAK, true);
  $urdalSatuanPengirim = $isSatlakPengirim ? \App\Models\Satuan::where('kode', 'URDAL')->first() : null;
  $satuanTembusanPilihanSatlak = $isSatlakPengirim
      ? $satuanSuratTujuanPilihan->reject(fn($st) => in_array(strtoupper($st->kode ?? ''), ['DANPUS', 'WADAN'], true))
      : $satuanSuratTujuanPilihan;
@endphp<div class="report-modal" id="kirimSuratModal"><div class="report-modal-card" style="width:min(860px,100%)"><div class="report-modal-head"><div style="display:flex;align-items:flex-start;gap:12px;min-width:0"><span class="surat-modal-icon"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8Z"></path><path d="M14 2v6h6"></path><path d="M12 12v6"></path><path d="M9 15h6"></path></svg></span><span style="min-width:0"><h3 style="margin:0 0 4px">Buat Surat Baru</h3><p style="margin:0;font-size:12px;color:var(--text-muted)">Dari {{ $satuan->nama }}</p></span></div><button type="button" class="report-modal-close" id="kirimSuratClose" aria-label="Tutup"><svg viewBox="0 0 24 24" stroke-linecap="round" stroke-linejoin="round"><path d="M6 6l12 12M18 6L6 18"></path></svg></button></div><form method="POST" action="{{ route('laporan-surat.store') }}" enctype="multipart/form-data" id="kirimSuratForm">@csrf<div class="surat-form-grid"><div class="surat-form-col"><div class="surat-section-label">Informasi Surat</div><div class="form-field full"><label for="surat_tujuan_search">Tujuan</label>@if($isSatlakPengirim && $urdalSatuanPengirim)<input type="hidden" id="surat_tujuan" name="tujuan_satuan_id" value="{{ $urdalSatuanPengirim->id }}"><div style="padding:9px 12px;border:1px solid var(--border);border-radius:7px;background:var(--panel-alt);font-size:12.5px;font-weight:600;">{{ $urdalSatuanPengirim->nama }} <span style="font-weight:400;color:var(--text-muted);font-size:10.5px">(otomatis — Surat Keluar Satlak selalu ke Urdal)</span></div>@else<div class="surat-combobox" id="suratTujuanCombobox"><input type="hidden" id="surat_tujuan" name="tujuan_satuan_id"><input type="text" id="surat_tujuan_search" autocomplete="off" placeholder="Ketik nama atau satuan tujuan..."></div><script>window.__suratTujuanOptions=@json($__suratTujuanOptions);</script>@endif</div><div class="form-field full"><label for="surat_perihal">Perihal</label><input id="surat_perihal" name="perihal" maxlength="255" required autocomplete="off" placeholder="Judul singkat surat"></div><div class="form-field full"><label for="surat_kategori">Kategori</label><input id="surat_kategori" name="kategori" maxlength="255" required autocomplete="off" placeholder="Contoh: Undangan, Pemberitahuan, Koordinasi"></div><div class="form-field full"><label>Jenis Surat</label><div class="priority-toggle"><label class="priority-option prio-rendah"><input type="radio" name="prioritas" value="Rendah" required><span>Rendah</span></label><label class="priority-option prio-sedang"><input type="radio" name="prioritas" value="Sedang" required><span>Sedang</span></label><label class="priority-option prio-tinggi"><input type="radio" name="prioritas" value="Tinggi" required><span>Tinggi</span></label></div></div><div class="form-field full"><label for="surat_deskripsi">Isi Ringkasan Surat</label><textarea id="surat_deskripsi" name="deskripsi" required placeholder="Tuliskan isi surat yang ingin disampaikan..."></textarea></div><div class="form-field full"><label>Tembusan Surat <span style="font-weight:400;text-transform:none;letter-spacing:0;color:var(--text-muted);font-size:10.5px">(opsional)</span></label><div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(140px,1fr));gap:6px;max-height:110px;overflow-y:auto;padding:8px;background:var(--panel-alt);border:1px solid var(--border);border-radius:7px;">@foreach($satuanTembusanPilihanSatlak as $st)<label style="display:flex;align-items:center;gap:6px;font-size:11.5px;cursor:pointer"><input type="checkbox" name="tembusan[]" value="{{ $st->id }}" style="width:auto"><span>{{ $st->nama_singkat ?: $st->nama }}</span></label>@endforeach</div></div></div><div class="surat-form-col surat-form-col-lampiran"><div class="surat-section-label">Lampiran</div><div class="surat-lampiran-zone" id="suratLampiranZone"><input id="surat_lampiran" type="file" class="surat-lampiran-zone-input" name="lampiran" data-file-picker-ready="1"><div class="surat-lampiran-zone-prompt"><span class="surat-lampiran-zone-icon"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M8 17l4-4 4 4"></path><path d="M12 13v9"></path><path d="M20.39 18.39A5 5 0 0 0 18 9h-1.26A8 8 0 1 0 3 16.3"></path></svg></span><span class="surat-lampiran-zone-text"><span class="surat-lampiran-zone-text-main">Klik untuk pilih file atau drag &amp; drop</span><span class="surat-lampiran-zone-text-sub">Maksimal 10 MB</span></span></div></div><div class="lampiran-file-list" id="suratLampiranFileList"><div class="lampiran-file-list-empty" id="suratLampiranFileListEmpty">Belum ada file dipilih</div></div><div class="surat-lampiran-preview" id="suratLampiranPreview" hidden></div><span class="kirim-laporan-error" id="suratLampiranError" style="display:none"></span></div></div><div class="modal-actions"><button type="button" class="btn" id="kirimSuratCancel">Batal</button><button class="btn btn-primary" type="submit" id="kirimSuratSubmitBtn">Buat Surat</button></div></form></div></div><div class="confirm-overlay" id="konfirmasiBuatSuratOverlay"><div class="confirm-box" role="alertdialog" aria-modal="true" aria-labelledby="konfirmasiBuatSuratTitle"><div class="confirm-icon" style="background:var(--gold-dim);color:var(--gold-bright)"><svg viewBox="0 0 24 24" stroke-linecap="round" stroke-linejoin="round" fill="none" stroke-width="1.9"><rect x="6" y="4" width="12" height="17" rx="2"></rect><path d="M9 4h6"></path><path d="M9 10h6"></path><path d="M9 14h6"></path><path d="M9 18h3"></path></svg></div><h3 id="konfirmasiBuatSuratTitle">Kirim Surat Baru?</h3><p>Pastikan data yang kamu isi sudah benar. Surat yang sudah dikirim tidak dapat diedit lagi.</p><div class="confirm-actions"><button type="button" class="btn" id="konfirmasiBuatSuratBatal">Batal</button><button type="button" class="btn btn-primary" id="konfirmasiBuatSuratYa">Ya, Kirim</button></div></div></div>
@include('siberad.dashboards.partials.surat-detail-modal')
@include('siberad.dashboards.partials.surat-teruskan-modal')
@include('siberad.dashboards.partials.surat-balasan-modal')
  ...
  function initMonitorSatlakFilter(prefix){
    ...
    function apply(){
      ...
      cards.forEach(function(card){
        ...
        var matchPr=!pr||card.dataset.prioritas===pr;
    ...
    prioritasSelect&&prioritasSelect.addEventListener('change',apply);
  }
  ['kal','dak','sisos'].forEach(initMonitorSatlakFilter);
})();
...
document.getElementById('kirimSuratOpen')?.addEventListener('click',()=>{const m=document.getElementById('kirimSuratModal');if(!m)return;void m.offsetHeight;m.classList.add('open');});document.getElementById('kirimSuratClose')?.addEventListener('click',()=>document.getElementById('kirimSuratModal')?.classList.remove('open'));document.getElementById('kirimSuratCancel')?.addEventListener('click',()=>document.getElementById('kirimSuratModal')?.classList.remove('open'));document.addEventListener('keydown',e=>{if(e.key==='Escape'&&document.getElementById('kirimSuratModal')?.classList.contains('open'))document.getElementById('kirimSuratModal').classList.remove('open')});(function(){const group=document.getElementById('tembusanKeGroup');if(!group)return;const boxes=group.querySelectorAll('.tembusan-ke-checkbox');const MAKS_TEMBUSAN=1;const errEl=document.getElementById('tembusanKeError');function sync(){const checked=Array.from(boxes).filter(b=>b.checked).length;boxes.forEach(function(b){b.disabled=!b.checked&&checked>=MAKS_TEMBUSAN});if(errEl)errEl.style.display=checked>0?'none':'';}boxes.forEach(function(b){b.addEventListener('change',sync)});sync();window.validasiTembusanKe=function(){const checked=Array.from(boxes).filter(b=>b.checked).length;if(checked===0){if(errEl){errEl.style.display='';errEl.textContent='Pilih minimal 1 satuan tembusan.'}return false;}if(errEl)errEl.style.display='none';return true;};})();
(function(){
  ...
  form.addEventListener('submit',function(e){
    ...
    // kirim.
    // Validasi tembusan WAJIB diisi (alur baru)
    if(typeof window.validasiTembusanKe==='function'&&!window.validasiTembusanKe()){
      document.getElementById('tembusanKeField')?.scrollIntoView({behavior:'smooth',block:'center'});
      return;
    }
...
(function(){
  ...
  // Terbaru" SENGAJA di-tick di sini (bukan ikut dikirim server tiap poll)
  // -- lihat penjelasan lengkap di komentar atas
  ...
  // KPI/donut/surat/kendala di sini nunggu 2 detik dulu sebelum sempat
  // nyegerin diri. lastFreshHtml/dst di atas udah di-seed dari HTML render
  // server, jadi panggilan langsung ini aman (gak flicker kalau belum ada
  // perubahan) -- cuma mempercepat begitu ADA perubahan yang kejadian pas
  // Satuan lagi transisi ke tab ini (audit polling 2026-09-14).
  syncSatuanKpis();
