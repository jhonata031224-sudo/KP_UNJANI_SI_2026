...
@if($isPenerimaTembusan)<section id="laporan-tembusan" class="tab-panel deadline-sender-section"><div class="report-card"><div class="panel-head"><div><h2>Tembusan Kendala</h2><p>Info koordinasi dari Kasansi (Sansidam) yang menembuskan laporan kendalanya ke {{ $satuan->nama }}. Ini bukan laporan yang perlu Anda putuskan -- keputusan tetap ada di Danpus.</p></div></div></div><div class="kcard-grid" id="kcard-grid-tembusan">@forelse($tembusanMasuk as $t)@include('siberad.dashboards.partials.laporan-kendala-tembusan-card', ['t' => $t])@empty<div class="kcard-empty"><svg viewBox="0 0 24 24" width="38" height="38" fill="none" stroke="var(--text-dim)" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><rect x="6" y="4" width="12" height="17" rx="2"></rect><path d="M9 4h6"></path><path d="M9 10h6"></path><path d="M9 14h6"></path><path d="M9 18h3"></path></svg><div class="kcard-empty-title">Belum ada tembusan kendala</div><div class="kcard-empty-sub">Tembusan dari Kasansi akan muncul di sini.</div></div>@endforelse</div></section>@endif
