...
<title>Staff Direktur — SIBERAD</title>
      ...
      <section class="tab-panel active" data-tab-panel="ringkasan">
        ...
        <div class="panel chart-box">
          <div class="chart-box-head-row">
            <div><h3 style="font-family:var(--display);font-size:17px;font-weight:700;">Analitik Koordinasi Antar Satlak</h3><p style="font-size:12px;color:var(--text-muted);margin-top:2px;">Jenis koordinasi yang ditangani, status koordinasi berjalan, dan laporan yang diteruskan ke WADAN.</p></div>
          ...
          <div class="chart-box-grid" id="chartBoxGrid">
            ...
            <div class="chart-mini">
              <div class="chart-mini-head">
                <h4>Koordinasi per Jenis</h4><p>Permintaan Personel, Jadwal, Insiden, dsb.</p>
              </div>
              <div class="chart-wrap"><canvas id="chartKoordinasiJenis"></canvas></div>
            </div>

            <div class="chart-mini">
              <div class="chart-mini-head">
                <h4>Status Koordinasi</h4><p>Menunggu Binfung, Berlangsung, Selesai.</p>
              </div>
              <div class="chart-wrap"><canvas id="chartStatusKoordinasiSdir"></canvas></div>
            </div>

            <div class="chart-mini">
              <div class="chart-mini-head">
                <h4>Laporan Diteruskan ke WADAN</h4><p>Menunggu vs Diteruskan.</p>
              </div>
              <div class="chart-wrap"><canvas id="chartStatusDiteruskan"></canvas></div>
            </div>
          </div>
          <p class="chart-legend-note">Ganti jenis grafik lewat dropdown di kanan atas — pilihan selain "Batang" akan otomatis memisah tiap grafik menjadi tampilan yang lebih besar. Filter tanggal masih simulasi proporsional karena histori koordinasi per tanggal belum tersambung ke database.</p>
        </div>
...
{{-- ===== ANALITIK KOORDINASI ANTAR SATLAK (RINGKASAN) ===== --}}
<script>
(function () {
  var canvasCheck = document.getElementById('chartKoordinasiJenis');
  ...
  var koordinasiJenis = @json($koordinasiPerJenis ?? []);
  var statusKoordinasiSdir = @json($statusKoordinasiSdir ?? []);
  var statusDiteruskan = @json($statusLaporanDiteruskan ?? []);
  ...
  function drawKoordinasiJenis(type, data) {
    var t = type === 'bar' ? 'doughnut' : type;
    renderChart(
      'chartKoordinasiJenis', t,
      data.map(function (s) { return s.jenis; }),
      data.map(function (s) { return s.jumlah; }),
      [cGold, cGreen, cRed, cAmber],
      { label: 'Jumlah Koordinasi' }
    );
  }

  function drawStatusKoordinasiSdir(type, data) {
    renderChart(
      'chartStatusKoordinasiSdir', type,
      data.map(function (s) { return s.label; }),
      data.map(function (s) { return s.jumlah; }),
      type === 'line' || type === 'radar' ? cGold : [cAmber, cAmber, cGreen],
      { label: 'Jumlah', lineColor: cGold }
    );
  }

  function drawStatusDiteruskan(type, data) {
    renderChart(
      'chartStatusDiteruskan', type,
  ...
  function redrawAll() {
    ...
    drawKoordinasiJenis(type, scaleFactor(koordinasiJenis, factor, 'jumlah'));
    drawStatusKoordinasiSdir(type, scaleFactor(statusKoordinasiSdir, factor, 'jumlah'));
    drawStatusDiteruskan(type, scaleFactor(statusDiteruskan, factor, 'jumlah'));
  }
