...
<div class="pimp-page"><section id="dashboard" class="tab-panel active"><div class="pimp-hero"><div class="pimp-eyebrow">{{ $pengaturan->namaSistem() }} // {{ $satuan->kode }}</div><h1>{{ $satuan->nama }}</h1><p>{{ now()->translatedFormat('l, d F Y') }}</p>@include('siberad.dashboards.partials.dash-hero-deco', ['icon' => 'rose'])</div>@php
