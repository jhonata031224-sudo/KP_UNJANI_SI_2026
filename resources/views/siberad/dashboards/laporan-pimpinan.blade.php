...
#kirimSuratModal .form-field label{font-size:11px;font-weight:700;color:var(--text-muted);text-transform:uppercase;letter-spacing:.05em}
...
<section id="kendala-kasansi" class="tab-panel"><div class="section-block"><div class="section-head-clean"><div><h2>Kendala Kasansi</h2><p>Laporan rutin/kendala yang dikirim langsung oleh satuan Kasansi (21 Sansidam) kepada Danpus, tanpa lewat Permintaan Laporan.</p></div></div><div class="kcard-grid" id="kcard-grid-masuk">@forelse($kendalaMasuk as $k)@include('siberad.dashboards.partials.kendala-kasansi-row', ['k' => $k])@empty<div class="kcard-empty"><svg viewBox="0 0 24 24" width="38" height="38" fill="none" stroke="var(--p-muted)" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M10.29 3.86 1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg><div class="kcard-empty-title">Belum ada laporan kendala</div><div class="kcard-empty-sub">Kendala dari satuan Kasansi akan muncul di sini.</div></div>@endforelse</div></div></section>
<section id="arsip-kendala-kasansi" class="tab-panel"><div class="section-block"><div class="section-head-clean"><div><h2>Arsip Kendala Kasansi</h2><p>Laporan kendala Kasansi yang sudah dikonfirmasi Danpus dan diarsipkan — tidak lagi tampil di daftar Kendala Kasansi yang masih aktif.</p></div></div><div class="kcard-grid" id="kcard-grid-arsip-pimpinan">@forelse($kendalaArsip as $k)<div class="kcard" data-search="{{ strtolower(($k->satuan->nama ?? '').' '.$k->perihal) }}" data-prioritas="{{ $k->prioritas }}"><div class="kcard-header"><div class="kcard-meta"><span class="priority-tag prio-{{ strtolower($k->prioritas) }}">{{ $k->prioritas }}</span><span class="satuan-pill">{{ $k->satuan->kode ?? $k->satuan->nama ?? '-' }}</span></div><span class="status-pill ok">Dikonfirmasi</span></div><div class="kcard-body"><div class="kcard-sender">{{ $k->satuan->nama ?? '-' }}</div><div class="kcard-perihal">{{ $k->perihal }}</div>@if($k->kategori)<div class="kcard-kategori">{{ $k->kategori }}</div>@endif</div><div class="kcard-confirm-info"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 8v13H3V8"/><path d="M23 3H1v5h22V3z"/><path d="M10 12h4"/></svg><span>Dikonfirmasi {{ $k->confirmed_at?->translatedFormat('d M Y, H:i') ?? '-' }} oleh <strong>{{ $k->confirmedBy->name ?? '-' }}</strong></span></div><div class="kcard-footer"><span class="kcard-date"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="9"/><path d="M12 7v5l3 3"/></svg>{{ $k->created_at->translatedFormat('d M Y, H:i') }}</span><div class="kcard-actions"><button type="button" class="kcard-btn kcard-btn-detail" onclick="openReportDetail(this)" data-pengirim="{{ e($k->satuan->nama ?? '-') }}" data-tujuan="{{ e($satuan->nama) }}" data-perihal="{{ e($k->perihal) }}" data-prioritas="{{ e($k->prioritas) }}" data-proyek="{{ e($k->kategori ?? '-') }}" data-tanggal="{{ e($k->created_at->translatedFormat('d M Y H:i')) }}" data-deskripsi="{{ e($k->deskripsi) }}" data-kendala="{{ e($k->catatan ?? '') }}" data-lampiran="{{ $k->lampiran_path ? collect([['url' => asset('storage/'.$k->lampiran_path), 'nama' => basename($k->lampiran_path)]])->toJson() : '[]' }}" data-kendala-report="1" data-readonly="1" data-readonly-text="Kendala ini sudah dikonfirmasi &amp; diarsipkan pada {{ e($k->confirmed_at?->translatedFormat('d M Y H:i')) }}."><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>Detail</button><button type="button" class="kcard-btn kcard-btn-reject" onclick="bukaHapusArsipKendala('{{ route('laporan-kendala.destroy', $k) }}','{{ csrf_token() }}','{{ e($k->perihal) }}')"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14H6L5 6"/><path d="M10 11v6M14 11v6"/><path d="M9 6V4h6v2"/></svg>Hapus</button></div></div></div>@empty<div class="kcard-empty"><svg viewBox="0 0 24 24" width="38" height="38" fill="none" stroke="var(--p-muted)" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M21 8v13H3V8"/><path d="M23 3H1v5h22V3z"/><path d="M10 12h4"/></svg><div class="kcard-empty-title">Belum ada kendala yang diarsipkan</div><div class="kcard-empty-sub">Kendala Kasansi yang sudah dikonfirmasi Danpus akan muncul di sini.</div></div>@endforelse</div></div></section>
