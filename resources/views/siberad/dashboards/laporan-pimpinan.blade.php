...
.danpus-task-row{display:flex;flex-direction:column}.danpus-task-row-inputs{display:flex;gap:8px;align-items:center}
...
(function(){
  ...
  (function(){
    ...
    function makeTaskRow(){
      ...
      row.innerHTML='<div class="danpus-task-row-inputs"><input type="text" name="tasks[]" required autocomplete="off" maxlength="255" placeholder="Contoh: Kumpulkan data insiden minggu ini"><button type="button" class="danpus-task-remove" aria-label="Hapus task">&times;</button></div>';
      wireRequiredValidation(row.querySelector('input'));
      return row;
    }
    ...
    function wireRequiredValidation(input){
      ...
      // Task row (.danpus-task-row) itu flex-column berisi
      // .danpus-task-row-inputs (baris input+tombol hapus) -- pesannya
      // dijadiin ANAK KEDUA dari .danpus-task-row (bukan sibling lepas di
      // #danpusRequestTaskList), soalnya kalau lepas: (1) klik "Hapus task"
      // cuma me-remove .danpus-task-row-nya, pesan errornya ketinggalan
      // "yatim" & nempel keliatan terus walau baris/tasknya udah diganti,
      // dan (2) itu bikin #danpusRequestTaskList.children.length ikut
      // ke-hitung pesan error (bukan cuma baris beneran), ngerusak
      // pengecekan "minimal 1 baris" di tombol Hapus. Nempelin pesannya
      // sebagai anak .danpus-task-row otomatis ke-hapus bareng pas
      // barisnya di-remove, dan children count tetap akurat.
      const taskRow=input.name==='tasks[]'?input.closest('.danpus-task-row'):null;
      const anchor=deadlinePart||taskRow||input.closest('.priority-toggle')||input;
      const appendAsChild=deadlinePart||taskRow;
      let msg=appendAsChild?anchor.querySelector(':scope > .danpus-request-error'):anchor.nextElementSibling;
      if(!msg||!msg.classList.contains('danpus-request-error')){
        msg=document.createElement('span');
        msg.className='danpus-request-error';
        msg.style.display='none';
        if(appendAsChild)anchor.appendChild(msg);
