...
(function(){
  ...
  (function(){
    ...
    // Tiap kategori (Unsur Pelayanan/Unsur Pembantu Pimpinan/Direktorat/
    // Satlak/Kasansi) punya tombol "Pilih Semua" sendiri di dalam grup-nya
    // -- label tombol grup itu HARUS ikut disinkronkan setiap kali tombol
    // global ATAU checkbox satuan mana pun berubah, biar labelnya (Pilih
    // Semua/Batalkan Semua) selalu cocok sama isi kategori itu sendiri,
    // bukan status keseluruhan.
    const groupSelectAllBtns=()=>Array.from(modal?.querySelectorAll('.danpus-request-selectall-group')||[]);
    ...
    function syncGroupSelectAllLabels(){
      groupSelectAllBtns().forEach(btn=>{
        const boxes=satuanChecks(btn.dataset.kategori);
        const allChecked=boxes.length>0&&boxes.every(b=>b.checked);
        btn.textContent=allChecked?'Batalkan Semua':'Pilih Semua';
      });
    }
    selectAllBtn?.addEventListener('click',()=>{
      ...
      syncSelectAllLabel();syncGroupSelectAllLabels();syncSatuanProxy();
    });
    modal?.addEventListener('click',e=>{
      const groupBtn=e.target.closest('.danpus-request-selectall-group');
      if(!groupBtn)return;
      const boxes=satuanChecks(groupBtn.dataset.kategori);
      const allChecked=boxes.length>0&&boxes.every(b=>b.checked);
      boxes.forEach(b=>{b.checked=!allChecked});
      syncSelectAllLabel();syncGroupSelectAllLabels();syncSatuanProxy();
    });
    ...
    modal?.addEventListener('change',e=>{
      if(e.target.name==='tujuan_satuan_ids[]'){syncSelectAllLabel();syncGroupSelectAllLabels();syncSatuanProxy()}
    });
  })();
