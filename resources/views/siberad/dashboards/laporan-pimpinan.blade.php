...
(function(){
  ...
  function makeSatlakChart(id){const el=document.getElementById(id);if(!el||typeof Chart==='undefined')return;const inner=document.getElementById('satlakChartInner');if(inner){const wrapWidth=el.closest('.chart-box')?.clientWidth||0;const neededWidth=satlakLabels.length*46;inner.style.minWidth=Math.max(wrapWidth,neededWidth)+'px'}const barCtx=el.getContext('2d');const gradient=barCtx.createLinearGradient(0,0,0,el.height||260);gradient.addColorStop(0,'#6366f1');gradient.addColorStop(1,'#3b82f6');window.siberadCharts.push(new Chart(el,{type:'bar',data:{labels:satlakLabels,datasets:[{label:'Jumlah laporan',data:satlakTotals,backgroundColor:gradient,hoverBackgroundColor:'#4f46e5',borderRadius:7,maxBarThickness:42}]},options:{responsive:true,maintainAspectRatio:false,scales:{y:{beginAtZero:true,ticks:{precision:0},grid:{color:'rgba(127,127,127,.15)'}},x:{grid:{display:false},ticks:{autoSkip:false,maxRotation:60,minRotation:60}}},plugins:{legend:{display:false}}}}))}
