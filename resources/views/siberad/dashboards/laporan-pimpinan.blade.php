...
(function(){
  ...
  document.querySelectorAll('.side-link[href^="#"],.side-sub-link[href^="#"],.card-link[href^="#"]').forEach(a=>a.addEventListener('click',e=>{const id=a.getAttribute('href').slice(1);if(document.getElementById(id)){e.preventDefault();showSection(id,a)}}));
  ...
  document.getElementById('reportDetailModal')?.addEventListener('click',e=>{if(e.target.id==='reportDetailModal')e.currentTarget.classList.remove('open')});
  ...
  document.getElementById('hapusRiwayatModal')?.addEventListener('click',e=>{if(e.target.id==='hapusRiwayatModal')tutupHapusRiwayatPimpinan()});
  ...
  (function(){
    ...
    let danpusPickedDate=null;
    let danpusPickedTime=null;
    const deadlineHidden=document.getElementById('danpusRequestDeadline');
    requestForm?.addEventListener('submit',()=>{
      if(deadlineHidden&&danpusPickedDate&&danpusPickedTime){
        deadlineHidden.value=danpusPickedDate+'T'+danpusPickedTime;
      }
    });
    ...
    // ---- Kalender tanggal ----
    (function(){
      const input=document.getElementById('danpusRequestDeadlineDate');
      const proxy=document.getElementById('danpusRequestDeadlineDateProxy');
      const picker=document.getElementById('danpusDatePicker');
      const panel=document.getElementById('danpusDeadlineCalendar');
      const daysEl=document.getElementById('danpusCalendarDays');
      const titleEl=document.getElementById('danpusCalendarTitle');
      const confirmBtn=document.getElementById('danpusCalendarConfirm');
      ...
      // panel (belum tentu final) -- baru dipindah ke danpusPickedDate (nilai
      ...
      const backdrop=document.getElementById('danpusPickerBackdrop');
      // .danpus-request-form-card punya `transform` buat animasi buka/tutup
      // modal -- itu bikin SEMUA descendant position:fixed di dalamnya
      // (termasuk panel ini) jadi nempel relatif ke form-card itu, BUKAN ke
      // viewport (aturan CSS: ancestor ber-transform jadi containing block
      // buat fixed descendant). Akibatnya panel ke-skip ke-stacking-context
      // form-card sendiri dan kalah tampil di bawah backdrop, jadi nggak
      // kesentuh. "Dipindah" jadi anak langsung <body> pas dibuka supaya
      // benar-benar lepas dari jebakan itu.
      function open(){document.body.appendChild(panel);panel.classList.add('open');backdrop?.classList.add('open');stagedDate=danpusPickedDate;if(confirmBtn)confirmBtn.disabled=!stagedDate;render()}
      ...
      confirmBtn?.addEventListener('click',e=>{
        ...
        danpusPickedDate=stagedDate;
        const [y,m,d]=stagedDate.split('-').map(Number);
        input.value=d+' '+monthNames[m-1]+' '+y;
        markSelected();
        close();
      });
      document.getElementById('danpusCalendarPrev')?.addEventListener('click',e=>{e.stopPropagation();viewM--;if(viewM<0){viewM=11;viewY--}render()});
      document.getElementById('danpusCalendarNext')?.addEventListener('click',e=>{e.stopPropagation();viewM++;if(viewM>11){viewM=0;viewY++}render()});
      document.getElementById('danpusCalendarClose')?.addEventListener('click',e=>{e.stopPropagation();close()});
    ...
    (function(){
      const input=document.getElementById('danpusRequestDeadlineTime');
      const proxy=document.getElementById('danpusRequestDeadlineTimeProxy');
      const picker=document.getElementById('danpusTimePicker');
      const panel=document.getElementById('danpusDeadlineClock');
      const hourWheel=document.getElementById('danpusWheelHour');
      const minuteWheel=document.getElementById('danpusWheelMinute');
      const confirmBtn=document.getElementById('danpusClockConfirm');
      if(!panel||!input||!hourWheel||!minuteWheel)return;
      const ITEM_H=36;
      ...
      function open(){
        ...
        const now=new Date();
        const [hh,mm]=(danpusPickedTime||'').split(':');
      ...
      confirmBtn?.addEventListener('click',e=>{
        ...
        danpusPickedTime=stagedHour+':'+stagedMinute;
        input.value=stagedHour+':'+stagedMinute;
        markSelected();
        close();
      });
      document.getElementById('danpusClockClose')?.addEventListener('click',e=>{e.stopPropagation();close()});
      panel.addEventListener('click',e=>e.stopPropagation());
      document.addEventListener('click',close);
    })();
    ...
    // bukan numpuk di tiap opsi.
    requestForm?.querySelectorAll('input[required],textarea[required]').forEach(input=>{
      ...
      const deadlinePart=input.closest('.danpus-deadline-part');
  ...
  const statusData={menunggu:{{ $laporanPimpinanSatlak->filter(fn($l)=>$l->status==='Menunggu')->count() }},disetujui:{{ $laporanPimpinanSatlak->filter(fn($l)=>str_contains(strtolower((string)$l->status),'setuj') || str_contains(strtolower((string)$l->status),'diterima'))->count() }},ditolak:{{ $laporanPimpinanSatlak->filter(fn($l)=>str_contains(strtolower((string)$l->status),'tolak'))->count() }}};
  window.siberadCharts = window.siberadCharts || [];
  function makeStatusChart(id){const el=document.getElementById(id);if(!el||typeof Chart==='undefined')return;window.siberadCharts.push(new Chart(el,{type:'doughnut',data:{labels:['Disetujui','Ditolak','Menunggu'],datasets:[{data:[statusData.disetujui,statusData.ditolak,statusData.menunggu],backgroundColor:['#22c55e','#ef4444','#f59e0b'],borderColor:'transparent',borderWidth:2}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{position:'bottom'}}}}))}
