...
<section id="monitoring" class="tab-panel"><div class="section-block"><div class="section-head-clean"><div><h2>Log Aktivitas</h2><p>Ringkasan Seluruh Satuan. Pilih “Lihat Aktivitas” untuk membuka daftar laporan secara detail.</p></div></div><div class="satlak-grid">@forelse($monitoringPimpinanSatlak as $m)<article class="satlak-card"><div class="code">{{ $m['kode'] ?? 'SATLAK' }}</div><div class="name">{{ $m['nama'] }}</div><div class="total">{{ $m['total_permintaan'] }}</div><div class="caption">Total permintaan ditugaskan</div><div class="mini-stats"><div class="mini-stat ok"><strong>{{ $m['diterima'] }}</strong><span>Disetujui</span></div><div class="mini-stat bad"><strong>{{ $m['ditolak'] }}</strong><span>Ditolak</span></div><div class="mini-stat wait"><strong>{{ $m['menunggu'] }}</strong><span>Menunggu</span></div></div><a class="card-link" href="#satlak-{{ $m['id'] }}">Lihat Aktivitas</a></article>@empty<div class="muted">Belum ada data Satlak.</div>@endforelse</div></div></section>
