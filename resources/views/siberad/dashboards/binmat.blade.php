...
<title>Pembinaan Materil — SIBERAD</title>
    ...
    <div class="content">
      ...
      <section class="tab-panel active" data-tab-panel="dashboard">
        ...
        <div class="panel chart-box">
          <div class="chart-box-head-row">
            <div><h3 style="font-family:var(--display);font-size:17px;font-weight:700;">Analitik Inventaris & Pengadaan</h3><p style="font-size:12px;color:var(--text-muted);margin-top:2px;">Kondisi inventaris, jumlah unit per kategori, dan status permintaan pengadaan.</p></div>
          ...
          <div class="chart-box-grid" id="chartBoxGrid">
            ...
            <div class="chart-mini">
              <div class="chart-mini-head">
                <h4>Kondisi Inventaris</h4><p>Baik, Perlu Perawatan, Kritis.</p>
              </div>
              <div class="chart-wrap"><canvas id="chartKondisiInventaris"></canvas></div>
            </div>

            <div class="chart-mini">
              <div class="chart-mini-head">
                <h4>Unit per Kategori</h4><p>Perangkat jaringan, komputer, server, dsb.</p>
              </div>
              <div class="chart-wrap"><canvas id="chartInventarisKategori"></canvas></div>
            </div>

            <div class="chart-mini">
              <div class="chart-mini-head">
                <h4>Status Pengadaan</h4><p>Menunggu Persetujuan vs Disetujui.</p>
              </div>
              <div class="chart-wrap"><canvas id="chartStatusPengadaan"></canvas></div>
            </div>
          </div>
          <p class="chart-legend-note">Merah = kondisi kritis, butuh tindakan segera. Ganti jenis grafik lewat dropdown di kanan atas — pilihan selain "Batang" akan otomatis memisah tiap grafik menjadi tampilan yang lebih besar. Filter tanggal masih simulasi proporsional karena histori inventaris per tanggal belum tersambung ke database.</p>
        </div>
      ...
      <section class="tab-panel" data-tab-panel="tambah-laporan">
        <div class="section-head">
          ...
          <p>Catat kendala, kerusakan, atau perkembangan baru terkait material/perlengkapan.</p>
        </div>
        <div class="panel">
          <form class="form-grid" id="formTambahLaporan" style="padding:22px;" novalidate>
            <div class="form-field">
              <label for="itemTambahLaporan">Item Terkait</label>
              <select id="itemTambahLaporan" required>
            ...
            <div class="form-field full">
              ...
              <input id="perihalTambahLaporan" type="text" placeholder="Contoh: Kerusakan hardware pada 1 unit server" required>
            </div>
            <div class="form-field full">
      ...
      <section class="tab-panel" data-tab-panel="status-laporan">
          <div class="section-head">
            ...
            <p>Laporan kondisi material yang menunggu diteruskan ke WADAN.</p>
          </div>
          <div class="panel">
            <div class="tbl-wrap">
              <table class="dtbl">
      ...
      <section class="tab-panel" data-tab-panel="riwayat-laporan">
        <div class="section-head">
          ...
          <p>Log aktivitas material/perlengkapan yang pernah tercatat.</p>
        </div>
        <div class="panel">
          <div class="tbl-wrap">
            <table class="dtbl">
...
{{-- ===== ANALITIK INVENTARIS & PENGADAAN (RINGKASAN) ===== --}}
<script>
(function () {
  var canvasCheck = document.getElementById('chartKondisiInventaris');
  ...
  var kondisiInventaris = @json($kondisiInventaris ?? []);
  var inventarisKategori = @json($inventarisPerKategori ?? []);
  var statusPengadaan = @json($statusPengadaan ?? []);
  ...
  function drawKondisiInventaris(type, data) {
    var t = type === 'bar' ? 'doughnut' : type;
    renderChart(
      'chartKondisiInventaris', t,
  ...
  function drawInventarisKategori(type, data) {
    renderChart(
      'chartInventarisKategori', type,
      data.map(function (s) { return s.kategori; }),
      data.map(function (s) { return s.jumlah; }),
      type === 'line' || type === 'radar' ? cGold : cGold,
      { horizontal: type === 'bar', label: 'Jumlah Unit', lineColor: cGold }
    );
  }

  function drawStatusPengadaan(type, data) {
    renderChart(
      'chartStatusPengadaan', type,
      data.map(function (p) { return p.label; }),
      data.map(function (p) { return p.jumlah; }),
      type === 'line' || type === 'radar' ? cGold : [cAmber, cGreen],
      { label: 'Jumlah Permintaan', lineColor: cGold }
    );
  }
  ...
  function redrawAll() {
    ...
    drawKondisiInventaris(type, scaleFactor(kondisiInventaris, factor, 'jumlah'));
    drawInventarisKategori(type, scaleFactor(inventarisKategori, factor, 'jumlah'));
    drawStatusPengadaan(type, scaleFactor(statusPengadaan, factor, 'jumlah'));
  }
