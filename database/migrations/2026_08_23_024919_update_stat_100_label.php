...
{
    /**
     * Ganti label stat "100%" dari "Data Terpusat & Terverifikasi" jadi
     * "Transparan & Real-Time" di landing_content JSON baris lama.
     */
    public function up(): void
    {
        DB::table('pengaturans')->orderBy('id')->get(['id', 'landing_content'])->each(function ($row) {
            ...
            foreach ($decoded['stats'] as $i => $stat) {
                if (($stat['number'] ?? null) === '100%' && ($stat['label'] ?? null) === 'Data Terpusat & Terverifikasi') {
                    $decoded['stats'][$i]['label'] = 'Transparan & Real-Time';
                    $changed = true;
                }
            }
