  ...
  function syncMaknaLogoPrototype(){
    ...
    if(!document.getElementById('makna-logo-prototype-override')){
      ...
      style.textContent=`
        ...
        #maknaLogoOverlay .makna-logo-point.is-bottom{height:48px!important;left:66%!important;right:auto!important;top:91%!important;transform:translate(-50%,-50%)!important;}
        ...
        @media(max-width:760px){#maknaLogoOverlay{align-items:flex-start!important;padding:78px 16px 32px!important;overflow:auto!important;}#maknaLogoOverlay .makna-logo-stage{width:100%!important;height:auto!important;min-height:0!important;display:flex!important;flex-direction:column!important;align-items:center!important;gap:22px!important;}#maknaLogoOverlay .makna-logo-crest{position:relative!important;top:auto!important;left:auto!important;width:min(46vw,190px)!important;height:min(46vw,190px)!important;transform:none!important;margin:0 auto!important;}#maknaLogoOverlay .makna-logo-lines,#maknaLogoOverlay .makna-logo-anchor-dot{display:none!important;}#maknaLogoOverlay .makna-logo-point{position:relative!important;inset:auto!important;width:100%!important;max-width:100%!important;height:auto!important;transform:none!important;}#maknaLogoOverlay .makna-logo-point.is-bottom{width:100%!important;max-width:100%!important;left:auto!important;top:auto!important;transform:none!important;}}
      `;document.head.appendChild(style);
    }
    const layout={
      1:{side:'right',x:70,top:15,width:350},
      2:{side:'left',x:4,top:24,width:300},
      3:{side:'right',x:69,top:50,width:390},
      4:{side:'left',x:9,top:43,width:390},
      5:{side:'right',x:72,top:33,width:360},
      ...
      10:{side:'bottom',x:66,top:91,width:300}
    };
    const anchors={
      1:[49.23,29.99],
      2:[44.59,33.38],
    ...
    const redraw=()=>{
      ...
      qa('.makna-logo-point',stage).forEach((point,index)=>{
        ...
        let route;
        if(number===10||number===3||number===5){route=[[x1,y1],[x2,y2]];}
        else if([2,6,7,8,9].includes(number)){const direction=isLeft?1:-1;const bendX=x1+direction*5;route=[[x1,y1],[bendX,y1],[x2,y2]];}
        else{route=[[x1,y1],[x2,y2]];}
        svg.querySelectorAll('[data-ml-line="'+index+'"]').forEach(el=>el.remove());
        if(route.length===2){const line=document.createElementNS('http://www.w3.org/2000/svg','line');line.setAttribute('data-ml-line',String(index));line.setAttribute('x1',route[0][0]);line.setAttribute('y1',route[0][1]);line.setAttribute('x2',route[1][0]);line.setAttribute('y2',route[1][1]);line.setAttribute('stroke','#FF9800');line.setAttribute('fill','none');svg.appendChild(line);}
        else{const poly=document.createElementNS('http://www.w3.org/2000/svg','polyline');poly.setAttribute('data-ml-line',String(index));poly.setAttribute('points',route.map(p=>p.join(',')).join(' '));poly.setAttribute('stroke','#FF9800');poly.setAttribute('fill','none');svg.appendChild(poly);}
      });
    };
