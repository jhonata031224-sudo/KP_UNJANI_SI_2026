(function () {
  ...
  function installStyles() {
    if (document.getElementById('role-access-layout-style-v3')) return;
    var style = document.createElement('style');
    style.id = 'role-access-layout-style-v3';
    style.textContent = [
      ...
      '[data-tab-panel="role-akses"] .role-access-table td{padding:16px;vertical-align:top;border-bottom:1px solid var(--border-soft);background:var(--panel);box-sizing:border-box;min-width:0}',
      ...
      '[data-tab-panel="role-akses"] .role-access-table th:nth-child(1),[data-tab-panel="role-akses"] .role-access-table td:nth-child(1){width:24%;padding-left:20px;padding-right:14px}',
      '[data-tab-panel="role-akses"] .role-access-table th:nth-child(2),[data-tab-panel="role-akses"] .role-access-table td:nth-child(2){width:26%;padding-left:14px;padding-right:14px}',
      '[data-tab-panel="role-akses"] .role-access-table th:nth-child(3),[data-tab-panel="role-akses"] .role-access-table td:nth-child(3){width:50%;padding-left:14px;padding-right:20px}',
      ...
      '[data-tab-panel="role-akses"] .role-access-access-layout{display:flex;align-items:flex-start;justify-content:flex-start;gap:16px;min-width:0}',
      ...
      '[data-tab-panel="role-akses"] .role-access-save{width:auto;max-width:210px;min-width:176px;flex:0 0 auto;box-sizing:border-box;text-align:center;justify-content:center;line-height:1.35;white-space:normal;word-break:normal;align-self:start}',
      '[data-tab-panel="role-akses"] .role-access-empty{padding:10px 0;color:var(--text-dim);font-size:12px}',
      '@media(max-width:900px){[data-tab-panel="role-akses"] .role-access-table th,[data-tab-panel="role-akses"] .role-access-table td{padding:12px 10px}[data-tab-panel="role-akses"] .role-access-table th:nth-child(1),[data-tab-panel="role-akses"] .role-access-table td:nth-child(1){width:28%;padding-left:12px}[data-tab-panel="role-akses"] .role-access-table th:nth-child(2),[data-tab-panel="role-akses"] .role-access-table td:nth-child(2){width:28%}[data-tab-panel="role-akses"] .role-access-table th:nth-child(3),[data-tab-panel="role-akses"] .role-access-table td:nth-child(3){width:44%}[data-tab-panel="role-akses"] .role-access-access-layout{gap:12px}}',
      '@media(max-width:700px){[data-tab-panel="role-akses"] .role-access-table{table-layout:auto}[data-tab-panel="role-akses"] .role-access-table thead{display:none}[data-tab-panel="role-akses"] .role-access-table,[data-tab-panel="role-akses"] .role-access-table tbody,[data-tab-panel="role-akses"] .role-access-table tr,[data-tab-panel="role-akses"] .role-access-table td{display:block;width:100%!important}[data-tab-panel="role-akses"] .role-access-table tbody tr{padding:14px 0;border-bottom:1px solid var(--border-soft)}[data-tab-panel="role-akses"] .role-access-table tbody tr:last-child{border-bottom:0}[data-tab-panel="role-akses"] .role-access-table td{border:0!important;padding:7px 12px;background:transparent!important}[data-tab-panel="role-akses"] .role-access-access-layout{flex-direction:column;gap:12px}[data-tab-panel="role-akses"] .role-access-permissions{width:100%;max-width:none}[data-tab-panel="role-akses"] .role-access-save{width:100%;max-width:none;min-width:0}}'
    ].join('');
  ...
  function getRoleCode(row) {
    ...
    if (badge) return normalizeText(badge.textContent).toUpperCase();
    return '';
  }
  ...
  function buildFreshTable(section) {
    ...
    panels.forEach(function (panel, index) {
      ...
      if (title) {
        Array.prototype.forEach.call(title.childNodes, function (node) {
          if (node.nodeType === 3) roleName += ' ' + node.textContent;
        });
      ...
      rows.push({
        ...
        code: code,
        description: description ? normalizeText(description.textContent) : 'Tidak ada deskripsi.'
      });
    });
    ...
    rows.forEach(function (item) {
      ...
      if (item.save) {
        item.save.setAttribute('form', item.form.id);
  ...
  function normalizeExistingTable(section) {
    ...
    var headers = Array.prototype.map.call(table.querySelectorAll('thead th'), function (th) {
      return normalizeText(th.textContent).toLowerCase();
    });
    ...
    if (headers.length >= 4) {
      ...
      if (roleIdx >= 0 && descIdx >= 0 && permIdx >= 0 && actionIdx >= 0) {
        table.querySelectorAll('tbody tr').forEach(function (tr) {
          var cells = tr.children;
          if (!cells[roleIdx] || !cells[descIdx] || !cells[permIdx] || !cells[actionIdx]) return;

          var roleCodeNode = cells[roleIdx].querySelector('.role-access-code,.badge');
          var roleCode = roleCodeNode ? normalizeText(roleCodeNode.textContent).toUpperCase() : '';
          var form = cells[permIdx].querySelector('form');
          var button = cells[actionIdx].querySelector('button');
          var layout = document.createElement('div');
          layout.className = 'role-access-access-layout';

          if (form) {
            normalizeForm(form, button, form.id || ('role-access-form-legacy-' + Math.random().toString(36).slice(2)));
            applyRolePermissions(form, roleCode);
            layout.appendChild(form);
          }
          if (button) layout.appendChild(button);

          cells[permIdx].innerHTML = '';
          cells[permIdx].appendChild(layout);
          cells[actionIdx].remove();
        });

        var head = table.tHead && table.tHead.rows[0];
        if (head) {
          if (head.children[permIdx]) head.children[permIdx].textContent = 'Hak Akses Modul & Aksi';
          if (head.children[actionIdx]) head.children[actionIdx].remove();
        }
      }
    }
    ...
    table.querySelectorAll('tbody tr').forEach(function (row) {
      ...
      if (form) applyRolePermissions(form, code);
    });
    return true;
  }
  ...
  function initRoleAccessTable() {
    ...
    if (section.querySelector('.role-access-table, .role-akses-table')) {
      normalizeExistingTable(section);
    } else {
      buildFreshTable(section);
    }

    var table = section.querySelector('.role-access-table');
    if (!table) return false;
  ...
  function scheduleRoleInit() {
    initRoleAccessTable();
    [50,250,750,1500].forEach(function (delay) { window.setTimeout(initRoleAccessTable, delay); });
  }
  ...
  function run() {
    ...
    document.addEventListener('click', function (e) {
      if (e.target.closest && e.target.closest('[data-tab-link="role-akses"]')) {
        window.setTimeout(initRoleAccessTable, 0);
        window.setTimeout(initRoleAccessTable, 150);
      }
    });
  }
