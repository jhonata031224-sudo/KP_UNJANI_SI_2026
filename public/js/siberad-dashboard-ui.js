(function () {
  ...
  function initAdminActiveSessionsRealtime() {
    var sessionSection = document.querySelector('[data-tab-panel="sesi-aktif"]');
    var table = sessionSection ? sessionSection.querySelector('table.dtbl') : null;
    if (!sessionSection || !table || table.dataset.realtimeBound === '1') return;

    table.dataset.realtimeBound = '1';
    ...
    var intervalId = window.setInterval(async function () {
      ...
      updating = true;
      try {
        var response = await fetch('/dashboard?_sesi_realtime=' + Date.now(), {
          method: 'GET',
        ...
        var freshSection = doc.querySelector('[data-tab-panel="sesi-aktif"]');
        var freshTbody = freshSection ? freshSection.querySelector('table.dtbl tbody') : null;
        if (!freshTbody) return;

        if (freshTbody.innerHTML !== tbody.innerHTML) {
          tbody.innerHTML = freshTbody.innerHTML;
        }
      } catch (error) {
        // Realtime bersifat tambahan; kegagalan polling tidak boleh mengganggu halaman.
      } finally {
        updating = false;
      }
    }, 3000);
  ...
  function boot() {
    ...
    initAdminActiveSessionsRealtime();
  }
